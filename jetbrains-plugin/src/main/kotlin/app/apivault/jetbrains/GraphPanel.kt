...
class GraphPanel(private val project: Project) : JPanel(BorderLayout()) {
    ...
    private fun post(jsonPayload: String) {
        runJs("window.apivaultLoad(${mapper.writeValueAsString(jsonPayload)});")
    }
    ...
    private fun revealCredentialFlow(node: ApiVaultService.GraphNode) {
        ApplicationManager.getApplication().invokeLater {
            ...
            ApplicationManager.getApplication().executeOnPooledThread {
                val value = project.service<ApiVaultService>()
    ...
    private fun notify(content: String, type: NotificationType = NotificationType.INFORMATION) {
        NotificationGroupManager.getInstance()
            .getNotificationGroup("app.api-vault.jetbrains.notifications")
            .createNotification("API Vault", content, type)
            .notify(project)
    }
}
