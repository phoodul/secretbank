...
class SupplyChainPanel(private val project: Project) : JPanel(BorderLayout()) {
    ...
    private fun renderFromCache() {
        val cached = project.service<SecretbankService>().lastScan
        if (cached != null) {
    ...
    private fun runScan() {
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            val rep = project.service<SecretbankService>().scanSupplyChain(basePath)
    ...
    private class AdvisoryTableModel : AbstractTableModel() {
        ...
        private var rows: List<SecretbankService.MatchedAdvisory> = emptyList()

        fun setRows(r: List<SecretbankService.MatchedAdvisory>) {
            rows = r.sortedByDescending { rank(it.severity) }
            fireTableDataChanged()
        }
        fun getRow(i: Int): SecretbankService.MatchedAdvisory? = rows.getOrNull(i)
