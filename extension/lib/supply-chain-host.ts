...
//
// extension/lib/supply-chain-host.ts — M24-E Phase G-2-2
//
// SupplyChainBanner 를 Closed Shadow DOM 에 React root 로 마운트.
// D-3 save-banner-host 패턴 그대로 재사용.
//
// T3: closed shadow DOM — host JS/CSS 침범 차단. host 페이지 JS 가 banner DOM 접근 ❌.

import React from "react";
import { createRoot, type Root } from "react-dom/client";
import { SupplyChainBanner, type SupplyChainBannerProps } from "../components/SupplyChainBanner";

const HOST_ID = "secretbank-supply-chain-banner-host";

let activeRoot: Root | null = null;
let activeHost: HTMLElement | null = null;

/**
 * SupplyChainBanner 를 Closed Shadow DOM 에 마운트한다.
 *
 * @returns unmount 함수 (호출 시 banner 제거)
 */
export function mountSupplyChainBanner(
  props: SupplyChainBannerProps,
  ...
  root.render(React.createElement(SupplyChainBanner, props));
