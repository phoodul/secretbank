...
## [2026-07-01] **자유 형식 Secure Note(여러 줄 메모) 도입 보류 — "기타(Other) + 커스텀 타입명"으로 충분**

- 사용자 확인: API/PW 외 다양한 항목 입력은 필요하나, **여러 줄 메모/자유 양식(다중 커스텀 필드)은 불필요**.
- 결정: 기존 `CredentialKind::Other` + `custom_kind_label`(SSH key/Token/License/MCP 토큰 등 타입명 자유 입력) + 단일 value(+선택 보조값) 로 충분. 로드맵 **M24 Phase 3-B `secure_note`(여러 줄 노트)** 는 계속 보류.
- 영향: 인벤토리 생성 UI 현 상태 유지. 자유 양식 편집기(멀티라인/키-값 반복 필드)는 신규 착수 안 함.
