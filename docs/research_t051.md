# T051 구현 선행 조사 — SaaS 상태 RSS 클라이언트 (`secretbank-feeds/rss.rs`, `sources.rs`)
...
상태 페이지들은 User-Agent 를 엄격하게 검증하지 않지만 `secretbank/0.1.0` 으로 통일한다.
...
```
src-tauri/crates/secretbank-feeds/tests/fixtures/rss/
...
**A 전략 채택 권장.** feed-rs 의 chrono 는 `secretbank-feeds` crate 내부에서만 사용되고
...
```toml
# secretbank-feeds/Cargo.toml [dependencies]
feed-rs = "2.3.1"
```
