...
> 작성일: 2026-05-07  
> 목적: API Vault M24 Phase 2-2B (HIBP Pwned Passwords + 약한 비번 + 재사용 + 2FA 검출) implementator 사양 작성 기초 자료
...
- HTTPS TLS 1.2+ 필수
- `User-Agent` 헤더 — 문서상 "권장" 수준, 필수는 아님. 프로젝트 관례상 `api-vault/0.1.0` 사용
...
**API Vault 권고**: SHA-1 모드만 구현. NTLM은 데스크톱 앱 비번 매니저 유스케이스에 불필요.
...
**API Vault 구현 지시**: `Add-Padding: true` 헤더 항상 포함.
...
**API Vault 권고 임계값**: `score <= Score::Two` (= zxcvbn 2 이하) + 추가로 길이 < 8자 강제 체크
...
이 구분은 API Vault 구현에서도 그대로 채용할 수 있음:
...
**API Vault 전략 권고**: 모든 검사를 로컬 처리(이미 vault는 로컬) + HIBP Pwned Passwords(API 키 불필요)는 무료로 제공 가능. 이메일 breachedaccount는 이미 별도 HIBP API 키 필요 (기존 구현).
...
pub async fn check_password(
    ...
    let response = self.http
        ...
        .header("User-Agent", "api-vault/0.1.0")
        .send()
        .await?
        .text()
        .await?;
...
`api-vault-feeds/Cargo.toml` 또는 신규 `api-vault-security` crate에 추가:
...
- `secrecy = "0.10"` — 이미 있음 (`api-vault-crypto`)
- `zeroize = "1"` — 이미 있음
...
또는 별도 `api-vault-security` crate 분리 (의존성이 커질 경우).
...
`src-tauri/crates/api-vault-core/src/services/matcher.rs` 의 `MatchReason` enum에:
```rust
CompromisedPassword { exposure_count: u64 },
...
*이 문서는 implementator 가 Phase 2-2B 사양을 작성하는 데 필요한 모든 기초 자료를 포함합니다.*  
*보안 결정 B.1 (암호학 직접 구현 금지, 평문 메모리 시간 최소화, 평문 IPC 미통과, timing-safe 비교 필수)을 준수하는 구현 패턴만 제시하였습니다.*
