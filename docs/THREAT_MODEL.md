# Secretbank — Threat Model (STRIDE)
...
- **저장 위치**: `~/Library/Application Support/secretbank/vault.age` (macOS) / 동등 OS 경로
...
| :------------------- | :---------------------------------------------------------- | :--------------------------------------------------------- | :-------------------------------------- |
| TB1                  | Phishing app 가 Secretbank UI 위장 → 사용자 passphrase 탈취 | OS app 서명 (codesign / Authenticode), 공식 배포 채널 권장 | HIGH (앱 위장 일반적). 사용자 교육 필요 |
...
신용카드는 PCI-DSS 영역 secret 이지만, Secretbank 가 결제 처리 ❌ → vault 안에서만 저장. PCI scope 적용 안 됨 (PCI-DSS Self-Assessment Questionnaire P2PE 또는 SAQ A 영역 외).
...
_본 문서는 STRIDE 모델 기반 1차 위협 분석. 출시 전 외부 감사 결과로 갱신 필수._
_완화책 미적용 (잔여 HIGH) 항목은 Phase 3-A 사양 수립 시 우선 검토._
