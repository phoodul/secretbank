...
| 平台    | 方式                                                         |
| :------ | :----------------------------------------------------------- |
| Windows | `api-vault_x64-setup.exe` 或 `winget install api-vault`      |
| macOS   | `api-vault_universal.dmg` 或 `brew install --cask api-vault` |
| Linux   | `.deb` / `.AppImage` / `.rpm` 或 `snap install api-vault`    |

构建版本：https://github.com/phoodul/api-vault/releases
...
cargo install api-vault-cli      # 所有平台
```
...
# 将凭证注入环境变量后执行命令。
# 示例：apivault run prod-stripe -- npm run deploy
```

### 9.3 环境变量注入（`run`）

`apivault run` 仅将选中的凭证放入子进程的环境变量。通过 `apivault.json` 将凭证 ID 映射到环境变量名：
...
- 故意没有 `--print` 标志 — 不要 `echo` `apivault reveal` 的结果。
...
apivault mcp serve --port 3737  # SSE（Copilot Chat 等）
```
...
重启后，在聊天中：`@api-vault list openai`。
...
- 在 VS Code Marketplace 搜索 "API Vault"。
- 或在 Open VSX 搜索同名扩展。

### 11.2 命令（命令面板）

- `API Vault: List credentials`
- `API Vault: Reveal credential` — 输入密码 → 复制到剪贴板。
...
- Settings → **Export encrypted backup** → 生成 `.apivault-backup` 文件。
- 文件用主密码加密 — 可以安全地放在云端。
- 建议：每周一次 + 每次更改主密码后。
...
2. **错误的金库文件。** 默认路径：`~/.local/share/api-vault/vault.age`（Linux）、`~/Library/Application Support/api-vault/vault.age`（macOS）、`%APPDATA%\api-vault\vault.age`（Windows）。如果你迁移机器时没有复制此文件，那就是空的新金库。
...
| Linux (AppImage) | 先解压，二进制位于 `usr/bin/apivault`                 |
...
**Q. 我想贡献代码。**
A. https://github.com/phoodul/api-vault — 欢迎 issues 和 PRs。合并前需要 CLA。

**Q. 我发现了安全问题。**
A. 通过 PGP 加密邮件发送到 security@api-vault.app。90 天负责任披露。
