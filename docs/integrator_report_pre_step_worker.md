...
현재 `site/index.html` 의 다운로드 로직은 `api.github.com/repos/phoodul/api-vault/releases` API를 직접 호출하고 `asset.browser_download_url` (= `objects.githubusercontent.com` CDN) 로 사용자를 보낸다. `src-tauri/tauri.conf.json` 의 updater endpoint 도 `github.com/phoodul/api-vault/releases/download/v{{current_version}}/latest.json` 을 직접 참조한다. `site/latest.json` 의 `platforms[].url` 역시 `github.com` 직접 URL 이다. 사용자의 요구사항은 브라우저 주소창과 Tauri 자동 업데이트 흐름 어디에도 `github.com` 이 노출되지 않는 것이다. 이를 위해 Cloudflare Worker stream proxy를 `api-vault.app/download/*` 및 `api-vault.app/api/latest` 에 배치한다. Worker는 클라이언트에게는 `api-vault.app` 도메인만 노출하며, 내부적으로 GitHub Releases CDN 에서 파일을 stream-forward 한다.
...
- 기존 `ee/api-vault-relay/` 와 다른 관리 패턴 — Open Core 구조 불일치
...
- 기존 `ee/api-vault-relay/` 패턴과 완전 일관 (wrangler.toml + src/index.ts 구조)
- 향후 R2 미러링 등 EE 기능 추가 시 자연스러운 확장 경로
- Worker 독립 배포 가능 — Pages 재배포 없이 proxy 로직 변경 가능

**단점**:
- Pages 와 동일 도메인에서 path-specific intercept (`api-vault.app/download/*`) 는 Cloudflare 공식 문서에 명시되지 않은 패턴 (커뮤니티 확인, 출처 16)
- `wrangler deploy` 별도 실행 필요 — one-time setup 단계 추가
- Pages route 와 Worker route 중복 등록 시 디버깅 난이도 상승

**검증 필요**: deploy 후 `curl https://api-vault.app/download/v0.1.0-pre10/api-vault_0.1.0_x64-setup.exe -I` 로 200 응답 확인 필수. Pages 가 먼저 응답하면 404.
...
2. **"공식 문서 미기재" 리스크는 관리 가능하다.** Cloudflare 커뮤니티(출처 16)에서 동일 도메인 Worker routes + Pages 공존이 수년간 동작 확인되었다. Pages 와 Worker route 가 동일 path 에 겹치지 않는 한(`/download/*`, `/api/*` 만 Worker) Pages 서빙에 영향 없다. 실제로 `ee/api-vault-relay` 도 Workers 기반이며 Cloudflare 의존도는 이미 확립됨.
...
**완화**:
- `REPO = "phoodul/api-vault"` 하드코딩 — 사용자 입력에서 repo 를 추출하지 않음
- tag + filename 검증 통과 후 최종 URL: `https://github.com/phoodul/api-vault/releases/download/<tag>/<filename>` 형식만 fetch
- Worker 외부 fetch 대상을 `github.com` 과 자기 자신(`api-vault.app/latest.json`)으로 제한
...
**결론**: stream proxy 다운로드 자체는 GitHub rate limit 대상 아님. `/api/latest` 에서 Pages 정적 파일(`site/latest.json`)을 fetch 하는 경우도 rate limit 대상 아님 (`api-vault.app` 도메인, GitHub API 아님).
...
**권고**: 초기 구현에서는 KV 없이 `fetch("https://api-vault.app/latest.json")` 직접 호출. TTL 은 Cloudflare edge 캐시가 담당 (`Cache-Control: public, max-age=60`). KV 캐시는 `/api/latest` 응답 속도 최적화가 필요해지는 시점에 추가.
...
**주의**: main branch protection rule 이 활성화된 경우 `GITHUB_TOKEN` push 가 차단될 수 있음. 현재 API Vault repo 는 branch protection 없음 (solo developer, release 이력 확인). 이 가정이 변경될 경우 PAT 를 별도 secret 으로 설정 필요 (`GITHUB_PUSH_PAT`).
...
name = "download-proxy"
main = "src/index.ts"
compatibility_date = "2026-04-25"   # ee/api-vault-relay 와 통일
...
`account_id` 는 `ee/api-vault-relay/wrangler.toml` 에서 확인된 값을 그대로 사용한다. `zone_name` 은 `api-vault.app` — `zone_id` 로 대체 가능하나 `zone_name` 이 더 가독성이 좋다.
...
const ALLOWED_EXTS = [
  ".exe", ".msi", ".dmg", ".app.tar.gz",
  ".AppImage", ".deb", ".rpm", ".sig", ".json",
];
...
3. upstream fetch: `fetch(`${GITHUB_BASE}/${tag}/${filename}`, { redirect: "follow", headers: { "User-Agent": "api-vault-proxy/1.0", ...(rangeHeader ? { "Range": rangeHeader } : {}) } })`
4. upstream 실패 시 `502`
...
2. 실패(`!ok`) 시 `502`
3. 응답: `Content-Type: application/json`, `Cache-Control: public, max-age=60`, `Access-Control-Allow-Origin: https://api-vault.app`
...
2. `classify()` 의 URL 구성 수정 — `asset.browser_download_url` 대신 Worker URL 형식으로:
   ...
   // 변경 후
   url: `https://api-vault.app/download/${tag}/${asset.name}`,
   ```
...
```json
"url": "https://github.com/phoodul/api-vault/releases/download/v0.1.0-pre8/..."
```

**변경 목표**:
```json
"url": "https://api-vault.app/download/v0.1.0-pre8/..."
```
...
```json
"endpoints": [
  "https://github.com/phoodul/api-vault/releases/download/v{{current_version}}/latest.json"
]
```
...
**`src-tauri/tauri.conf.json` CSP 수정 확인**: 현재 CSP 의 `connect-src` 에 `https://api-vault.app` 가 이미 포함되어 있음 (`src-tauri/tauri.conf.json:22` 확인). `github.com` 항목 유지 여부는 Sub-task 5 에서 결정 — 현재 단계에서는 제거 불필요 (Tauri 앱 내부 로직에서 github.com 직접 호출이 있을 수 있음).
...
1. **One-time setup 섹션 — "4. Cloudflare Worker download-proxy 배포" 추가**:
   ...
   - `api-vault.app/download/*` + `api-vault.app/api/*` route 등록 확인 방법
...
4. **"Domain + landing" 섹션 (신규 또는 기존 갱신)**:
   - Cloudflare Pages (`api-vault.app/*`) + Worker routes (`/download/*`, `/api/*`) 공존 구조 다이어그램 추가
...
# 업데이트 체크 로그에서 api-vault.app/api/latest 호출 확인
```

### 6-3. Release pipeline 검증 (pre11 tag push 후)

1. `release.yml` 완료 후 `site/latest.json` 의 `platforms[].url` 이 `https://api-vault.app/download/v0.1.0-pre11/...` 형식인지 확인
2. main branch 의 commit history 에 `"chore(release): v0.1.0-pre11 latest.json 갱신 [skip ci]"` 커밋 확인
3. Cloudflare Pages 자동 재배포 완료 후 `curl https://api-vault.app/latest.json` 으로 pre11 버전 확인
4. pre10 설치 앱에서 업데이트 체크 → pre11 감지 → `https://api-vault.app/download/v0.1.0-pre11/...` 로 파일 다운로드 → 서명 검증 → 설치
...
**배경**: 현재 `ee/api-vault-relay` Worker 가 동일 계정에서 운영 중. Free plan (100k req/day) 이 dogfooding 단계에서는 충분. Paid ($5/월)는 10M req/month 포함.
...
**integrator 권고**: wrangler.toml 에 주석으로 미리 기록 (값 없이 이름만). `ee/api-vault-relay/wrangler.toml` 의 secrets 주석 패턴 그대로 따름.
