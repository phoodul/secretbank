...
| **M24** | **General password vault — Unified Bento Inventory** | T-24-A~E + Phase 1/1.5/2/3 | 5 + sub | 🔄 **Phase 1 ✅ + 1.5 ✅ + 2-1 ✅ + 2-2A ✅ + 2-2C ✅ + 2-3-a ✅ + 2-4-a ✅ + 2-4-d ✅ + 2-2B-1 ✅ + 2-2B-2 ✅ + 2-2B-3 ✅ (2026-05-07)** — type-agnostic bento card + value pair + URL auto-detect + HIBP breaches + issuer.domains + Incident.domain + KISA 5 RSS + Google CSV import + QuickAddDialog + apivault add CLI + PwnedPasswordsClient + security_check.rs + TwoFaDirectoryClient + **SQLite security_alerts 마이그레이션 (0011) + SecurityAlertRepo + TwoFaDirectoryCacheRepo + spawn_security_check_poller skeleton**. 다음: Phase 2-2B-4 (run_security_check Tauri command + Watchtower UI). |
