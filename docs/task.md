...
| **M24** | **General password vault — Unified Bento Inventory** | T-24-A~E + Phase 1/1.5/2/3 | 5 + sub | 🔄 **Phase 1 ✅ + 1.5 ✅ + 2-1 ✅ + 2-2A ✅ + 2-2C ✅ + 2-3-a ✅ + 2-4-a ✅ + 2-4-d ✅ + 2-2B-1 ✅ + 2-2B-2 ✅ + 2-2B-3 ✅ + 2-2B-4 ✅ (2026-05-07)** — backend 풀체인 완성: PwnedPasswordsClient + security_check.rs + TwoFaDirectoryClient + SQLite 0011 마이그레이션 + SecurityAlertRepo + scheduler skeleton + **4 Tauri commands (`run_security_check` concurrency 10 + `list_security_alerts` + `dismiss/undismiss`) + audit log 통합 (수동만, 요약만)**. 다음: Phase 2-2B-5 (WatchtowerPage UI + BentoCard 배지 + Settings opt-in + i18n 4 로케일 — 풀체인 완료). |
