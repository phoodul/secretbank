...
| **M24-E** | **Browser Extension (Phase A~F 풀구현 + Phase G 차별화)** | T-24-E-A1~F8 + G1-1~G5 | 53 sub-tasks | 🔄 **22/53 완료** — A 7/7 ✅ + B 10/10 ✅ + **C 5/8** (C-1 form-detector + C-2 SPA watcher + C-3 Shadow DOM + C-4 autofill + domain-match + C-5 trigger focus/hotkey) + GATE 1/2/2-bis + CI all green. **Phase G 신설 (2026-05-09)**: G-1~G-5 (모두 1P 미보유, M3/M5/M18/M20 재사용). 상세 = `docs/task_m24e.md` + `docs/task_m24e_phase_g.md`. 진입 순서: A → B → C → D → E → **G** → F-1 → F-2. **11주** 일정. |
