...
| **M24-E** | **Browser Extension (Phase A~F 풀구현)** | T-24-E-A1~F8 | 43 sub-tasks | 🔄 **10/43 완료** — GATE 1 + GATE 2 일괄 승인 (2026-05-09). **T-24-E-B5 완료 (2026-05-09)**: X25519 ECDH + XChaCha20-Poly1305 페어링 — Rust pairing.rs (15 tests, RFC 7748) + TS crypto.ts/pairing.ts (35 tests). 상세 명세 = `docs/task_m24e.md`. Phase A (WXT 모노레포) → B (NM Host + 페어링, HIGH 위험) → C (form 감지) → D (save dialog) → E (generator + Site Logo) → F (cross-browser + 스토어 + audit). 8주 일정. |
