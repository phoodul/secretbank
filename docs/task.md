...
| **M24-E** | **Browser Extension (Phase A~F 풀구현 + Phase G 차별화)** | T-24-E-A1~F8 + G1-1~G5 | 53 sub-tasks | 🔄 **18/53 완료** — A 7/7 ✅ + B 10/10 ✅ + **C-1 ✅** (form-detector autocomplete 우선순위 + 5 fixtures + 13 tests) + GATE 1/2/2-bis + CI all green. **Phase G 신설 (2026-05-09)**: G-1~G-5 (모두 1P 미보유, M3/M5/M18/M20 재사용). 상세 = `docs/task_m24e.md` + `docs/task_m24e_phase_g.md`. 진입 순서: A → B → C → D → E → **G** → F-1 → F-2. **11주** 일정. |
