...
| **M24-E** | **Browser Extension (Phase A~F 풀구현 + Phase G 차별화)** | T-24-E-A1~F8 + G1-1~G5 | 53 sub-tasks | 🔄 **17/53 완료** — A 7/7 ✅ + **B 10/10 ✅** (B-9 audit scope 문서 + B-10 3 OS smoke 시나리오 + Win11 자동 ping/pong PASS) + GATE 1/2/2-bis 일괄 승인. **Phase G 신설 (2026-05-09)**: G-1 Inline 의존성 mini-graph + G-2 Supply chain banner + G-3 Blast radius preview + G-4 MCP context push + G-5 RAILGUARD 인라인 hint (모두 1P 미보유, M3/M5/M18/M20 재사용). 상세 = `docs/task_m24e.md` + `docs/task_m24e_phase_g.md`. 진입 순서: A → B → C → D → E → **G** → F-1 → F-2. **11주** 일정. |
