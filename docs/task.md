...
| **M24-E** | **Browser Extension (Phase A~F 풀구현 + Phase G 차별화)** | T-24-E-A1~F8 + G1-1~G5 | 53 sub-tasks | ✅ **53/53 완료 — M24-E 클로즈 [2026-05-10]** — A 7/7 ✅ + B 10/10 ✅ + C 8/8 ✅ + D 6/6 ✅ + E 5/5 ✅ + G 10/10 ✅ + F 8/8 ✅ (F-3 Playwright + F-5 Mock NM + F-4 web-ext + F-1 Chrome + F-2 Firefox + F-7 Edge + F-6 Safari placeholder + F-8 audit placeholder) + GATE 1/2/2-bis. 남은 사용자 액션: Chrome $5 / Firefox 무료 / Edge 무료 제출 + Safari (Apple Dev $99/년 선택). 상세 = `docs/task_m24e.md`. |
...
| T020    | 도메인 모델 (secretbank-core)                                | 2026-04-22 | `57959f7` |
...
| T041    | `secretbank-core` 그래프 모델 (petgraph DiGraph)                 | 2026-04-23 | `5256f71` |
...
| T049    | NVD CVE API 2.0 클라이언트 (secretbank-feeds 크레이트 + governor rate limiter + wiremock 6 tests) | 2026-04-24 | `9a7895f` |
...
| T069    | secretbank-audit 크레이트 (append/verify + ed25519 + SHA-256 canonical + 7 tests)                                                                                              | 2026-04-25 | `79a8c1e` |
...
| T083-C  | OAuth(GitHub/Google) 2 커맨드 + tauri-plugin-deep-link `Secretbank://` scheme 등록 + on_open_url emit + tauri-plugin-opener 전환 + CSP 확장 + wiremock 5 회귀                                | 2026-04-27 | `e159415` |
...
| T084    | SignIn UI — `/auth/sign-in` (PasskeyButton + OAuthButton GitHub/Google + Keep offline) + use-deep-link-callback `Secretbank://auth/callback` 파서 + use-auth-session 훅 + Settings → CloudSyncSection 진입점 + i18n 4 로케일 + Vitest +19 (parser 4 / PasskeyButton 5 / OAuthButton 3 / SignInPage 4 / CloudSyncSection 3) — **M8 8/8 완료** | 2026-04-28 | `d619566` |
...
| 주제 | 커밋 해시 |
| :--- | :-------- |
| **I3 hotfix** — `useGithubIntegration` deep-link listener 표준화. `deep-link://github-callback` (lib.rs 가 emit 안 함, dead path) → `deep-link` 이벤트 + `Secretbank://github/callback` URL prefix 매칭. parseGithubCallbackUrl 헬퍼 + Vitest +8 + Setup URL 운영 가이드 강화 | `340e72c` |
...
### M24 Phase 2-2A-3a (2026-05-06, issuer.domains 컬럼 + preset seed)

| 주제 | 커밋 해시 |
| :--- | :-------- |
| **Phase 2-2A-3a** 마이그레이션 0009 + Issuer/IssuerInput.domains Vec<String> + IssuerRepo CRUD JSON 직렬화 + 10 preset domain 매핑 + frontend Issuer/IssuerYValue/issuerMapper sync + 전체 테스트 픽스처 보완 | `9bac675` |

### M24 Phase 2-2A-3b (2026-05-06, Incident.domain + MatchReason::Domain + subdomain-safe 매칭)
...
### M24 Phase 2-3-a-1 (2026-05-07, Google CSV 파서 + 단위 테스트)

| 주제 | 커밋 해시 |
| :--- | :-------- |
| **Phase 2-3-a-1** `import/csv_google.rs` 파서 (Chrome/Brave 5컬럼·Edge 3컬럼, header-based 자동 감지) + BOM 방어 + RFC 4180 escape + SecretBox<String> 즉시 래핑 + 빈 password 행 skip + `import/mod.rs` 신설 + `csv = "1"` workspace dep 추가 + 9개 단위 테스트 전원 PASS + clippy 0 warning | `15d2cc1` |

### M24 Phase 2-3-a-2 (2026-05-07, CSV row → DetectedKey 변환 + 도메인 매칭)
...
### M24 Phase 2-3-a-4 (2026-05-07, import_csv_commit 커맨드 + 부분 실패 처리)
...
### M24 Phase 2-4-a (2026-05-07, Cmd+K Quick Add 다이얼로그 + 클립보드 prefill)

| 주제 | 커밋 해시 |
| :--- | :-------- |
| **Phase 2-4-a** `actions.ts` `action.quick-add` 추가 (Zap 아이콘) + `QuickAddDialog.tsx` 신규 (5필드 경량 폼: URL/username/password/name/kind 토글 / mount 시 readText() → URL 패턴 일치 시 자동 채움 + "Auto-filled" 표시 / URL onChange → matchIssuerByUrl → issuer 이름 + api_key 자동 전환 / "전체 옵션 보기" onShowFullForm prefill 전달 / submit 직후 password state 비움) + `InventoryPage` `?action=quick-add` 파라미터 감지 + i18n `quickAdd.*` 22키 × 4 로케일 (en/ko/ja/zh) + Vitest 7 PASS (535 → 542). typecheck / lint / format:check / cargo 전원 통과. | `dfb9a57` |

### M24 Phase 2-4-d (2026-05-07, Secretbank add CLI 서브커맨드 + Secretbank_PASSPHRASE)

| 주제 | 커밋 해시 |
| :--- | :-------- |
| **Phase 2-4-d** `Command::Add` variant (url/user/pw/name/kind/env/json 7개 옵션, clap value_parser 유효성 검사) + `cmd_add()` 구현 (DB 확인 → password stdin 프롬프트 → URL→host → subdomain-safe issuer 자동 매칭 → issuer_id NOT NULL 폴백 → kind/env/name fallback chain → vault unlock → CredentialRepo::insert + put_secret + flush → text/JSON 출력) + `get_passphrase()` 헬퍼 추출 (Secretbank_PASSPHRASE env → 즉시 remove_var, 없으면 rpassword stdin) + `open_vault()` 에서 재사용 + `url = "2.5"` + `time` 의존성 추가 + 신규 테스트 11개 PASS (기존 3 포함 총 14 PASS). clippy 0 warning / fmt 통과. 서브커맨드 6 → 7. | `c041ee2` |

### M24 Phase 2-2B-1 (2026-05-07, HIBP Pwned Passwords range lookup 클라이언트)

| 주제 | 커밋 해시 |
| :--- | :-------- |
| **Phase 2-2B-1** `pwned_passwords.rs` 신규 (~407 라인) — `PwnedPasswordsClient` (`api.pwnedpasswords.com/range/{prefix}`) + `PwnedError` + `parse_range_response`. SHA-1 prefix 5자 (대문자 hex) k-anonymity lookup + `Add-Padding: true` 헤더 (Cloudflare/Troy Hunt 권고) + `subtle::ConstantTimeEq` timing-safe suffix 비교 (B.1-10) + `SecretBox<String>` expose_secret 스코프 최소화 (B.1-2) + suffix 길이 != 35자 행 skip guard fuzz-safe (B.1-4) + 에러 메시지 범용화 URL/credential 미노출 (B.1-9). 신규 의존성 `sha1 = "0.10"` (RustCrypto) + `subtle = "2.6"` (workspace 등록). wiremock 7 단위 테스트 PASS (T1 발견 / T2 미발견 / T3 padding / T4 헤더 / T5 prefix path / T6 fuzz-safe / T7 5xx). 기존 `HibpClient` 변경 없음. cargo test 79 passed / clippy 0 warning / fmt 통과. | `e26cc2d` |

### M24 Phase 2-2B-2 (2026-05-07, security_check 엔진 + 2fa.directory 클라이언트)
...
### M24 Phase 2-2B-4 (2026-05-07, security check Tauri commands + audit + concurrency 10)

| 주제 | 커밋 해시 |
| :--- | :-------- |
| **Phase 2-2B-4** `commands/security_check.rs` 신규 (~1005 라인, +12 단위 테스트) — 4 Tauri commands `tauri::generate_handler!` 등록. **`run_security_check(force_hibp: bool)`** — vault unlock 확인 → 모든 credential + 평문 password (SecretBox) 로드 → 로컬 4종 검사 (weak/reused/unsecured/missing 2FA) + `force_hibp=true` 일 때만 HIBP `tokio::sync::Semaphore` concurrency 10 (GATE 1-5) → `SecurityAlertRepo.replace_alerts_for_credential` 트랜잭션 → `AuditRepo.append` (B.1-6, GATE 1-6 수동만 + 요약만 credential_id 미포함) → `SecurityCheckSummary` 반환 (평문 미포함 B.1-3, `hibp_failed=true` 시 R3 부분 실패 처리). **`list_security_alerts(All / ByKind / ByCredential)`** 필터링. **`dismiss/undismiss_security_alert(alert_id)`** dismissed_at 토글. **R4 fallback**: `CredentialKind::Totp` variant 부재 (실제 = `ApiKey | Password`) → `secondary_value_ref.is_some() && secondary_label.contains("totp")` case-insensitive 로 TOTP 슬롯 판정. `SecurityCheckCommandError` 2 variant (`VaultLocked` / `Internal`) 범용화 (B.1-9). `ulid` workspace dev-deps 재사용. cargo test 230 passed / clippy 0 warning / fmt 통과. | `1dd89f4` |

### M24 Phase 2-2B-5 (2026-05-07, WatchtowerPage UI + BentoCard 배지 + Settings opt-in + i18n 4 로케일)
...
- **DoD**:
  ...
  - `crates/secretbank-core`, `secretbank-storage`, `secretbank-crypto`, `secretbank-audit`, `secretbank-feeds`, `secretbank-connectors`, `secretbank-railguard`, `secretbank-sync`, `secretbank-app` 9개 크레이트 생성 (stub lib.rs)
  - `secretbank-app` 이 기존 `src-tauri/src/lib.rs` 의 역할을 인계
...
- **DoD**:
  - `tauri-plugin-sql`, `tauri-plugin-clipboard-manager`, `tauri-plugin-shell`, `tauri-plugin-os`, `tauri-plugin-updater`, `tauri-plugin-notification`, `tauri-plugin-biometric`, `tauri-plugin-deep-link`, `tauri-plugin-http` 가 `secretbank-app/Cargo.toml` 에 있고 `lib.rs` 의 `Builder::default().plugin(...)` 체인에 등록됨
...
- **Files Touched**: `src-tauri/crates/secretbank-app/Cargo.toml`, `src-tauri/crates/secretbank-app/src/lib.rs`, `src-tauri/capabilities/default.json`, `package.json`
...
- **DoD**:
  - `crates/secretbank-storage/migrations/0001_init.sql` 에 `issuer`, `credential`, `project`, `deployment`, `usage`, `incident`, `incident_match`, `audit_log`, `device`, `sync_state`, `settings` 테이블 CREATE
  - 인덱스 7종 생성 (architecture.md 2.2)
  - `PRAGMA foreign_keys = ON`
  - `sqlx::migrate!` 런타임에서 동작
- **Files Touched**: `crates/secretbank-storage/migrations/0001_init.sql`, `crates/secretbank-storage/src/sqlite/mod.rs`
...
- **DoD**:
  - `crates/secretbank-storage/src/vault/mod.rs` 에 `trait VaultStorage` 선언:
...
- **Files Touched**: `crates/secretbank-storage/src/vault/mod.rs`, `crates/secretbank-storage/src/vault/error.rs`
...
- **DoD**:
  - `crates/secretbank-storage/src/vault/mock.rs` 에 `MockVaultStorage`
...
- **Files Touched**: `crates/secretbank-storage/src/vault/mock.rs`, `crates/secretbank-storage/Cargo.toml`
...
- **DoD**:
  - `crates/secretbank-storage/src/age_vault/mod.rs` 에 `AgeVaultStorage`
...
- **Files Touched**: `crates/secretbank-storage/src/age_vault/mod.rs`, `crates/secretbank-storage/Cargo.toml` (추가 의존성: `age`, `secrecy`, `zeroize`)
...
- **DoD**:
  - `crates/secretbank-crypto/src/kdf.rs`
...
- **Files Touched**: `crates/secretbank-crypto/src/kdf.rs`, `crates/secretbank-crypto/src/lib.rs`, `Cargo.toml`
...
- **DoD**:
  - `crates/secretbank-crypto/src/os_keyring.rs`
  ...
  - 서비스 이름: `com.phoodul.Secretbank`, account: `master:<user_id>`
  - Linux headless 에서 실패 시 `KeyringError::Unavailable` (호출부에서 폴백)
- **Files Touched**: `crates/secretbank-crypto/src/os_keyring.rs`
...
- **DoD**:
  - `crates/secretbank-storage/src/sqlite/repositories/{credential,issuer,project,deployment,usage,incident,audit,device,settings}.rs`
  - 각 레포: `insert`, `get_by_id`, `list(filter)`, `update`, `delete` 기본 CRUD
  - 도메인 모델 매핑: `sqlx::FromRow`
  - 시간 필드: `time::OffsetDateTime` → Unix ms (i64)
- **Files Touched**: `crates/secretbank-storage/src/sqlite/repositories/*.rs`
- **Tests**: Rust — 각 레포별 `tests/repo_{name}_test.rs` 에서 CRUD 라운드트립 (in-memory sqlite `sqlite::memory:`)

### T020. 도메인 모델 (`secretbank-core`)
...
- **DoD**:
  - `crates/secretbank-core/src/models/{issuer,credential,project,deployment,usage,incident,audit_log,device}.rs`
...
- **Files Touched**: `crates/secretbank-core/src/models/*.rs`, `crates/secretbank-core/src/lib.rs`
...
- **DoD**:
  - `crates/secretbank-app/src/commands/vault.rs`
...
- **Files Touched**: `crates/secretbank-app/src/commands/vault.rs`, `crates/secretbank-app/src/lib.rs` (invoke_handler 등록)
...
- **Goal**: 크리덴셜 CRUD IPC.
- **DoD**:
  - `crates/secretbank-app/src/commands/credentials.rs`
...
- **Files Touched**: `crates/secretbank-app/src/commands/credentials.rs`
...
- **DoD**:
  - `crates/secretbank-app/src/commands/clipboard.rs`
...
- **Files Touched**: `crates/secretbank-app/src/commands/clipboard.rs`
...
- **DoD**:
  ...
  - `crates/secretbank-app/src/setup.rs` 에 시드 로직
- **Files Touched**: `src/features/inventory/issuer-presets.ts`, `crates/secretbank-app/src/setup.rs`, `crates/secretbank-app/src/commands/issuer.rs` (list/get)
...
- **Files Touched**: `src/features/settings/SettingsPage.tsx`, `src/features/settings/use-settings.ts`, `crates/secretbank-app/src/commands/settings.rs`, `crates/secretbank-storage/src/sqlite/repositories/settings.rs` (이미 T019에 포함)
...
- **DoD**:
  - `crates/secretbank-connectors/src/env_scanner/mod.rs`
...
- **Files Touched**: `crates/secretbank-connectors/src/env_scanner/mod.rs`, `crates/secretbank-connectors/src/env_scanner/entropy.rs`, `crates/secretbank-connectors/src/env_scanner/parser.rs`, `Cargo.toml`
...
- **Goal**: 프론트엔드에서 스캔 호출.
- **DoD**:
  - `crates/secretbank-app/src/commands/scanner.rs`
...
- **Files Touched**: `crates/secretbank-app/src/commands/scanner.rs`, `src-tauri/capabilities/default.json`
...
- **DoD**:
  ...
  - `project_*` 커맨드 추가
- **Files Touched**: `src/features/projects/*.tsx`, `crates/secretbank-app/src/commands/projects.rs`
...
- **DoD**:
  ...
  - `deployment_*` 커맨드
- **Files Touched**: `src/features/projects/DeploymentSection.tsx`, `crates/secretbank-app/src/commands/deployments.rs`
- **Tests**: Vitest — CRUD
...
- **DoD**:
  ...
  - 목록에서 제거 버튼
  - `usage_*` 커맨드
- **Files Touched**: `src/features/inventory/UsageSection.tsx`, `crates/secretbank-app/src/commands/usages.rs`
...
- **DoD**:
  - `crates/secretbank-core/src/security_score.rs` — `fn score(cred: &Credential, usages: &[Usage]) -> ScoreBreakdown { total: u8, factors: Vec<ScoreFactor> }`
...
- **Files Touched**: `crates/secretbank-core/src/security_score.rs`, `src/features/inventory/SecurityDot.tsx`
...
### T041. `secretbank-core` 그래프 모델 (`petgraph`)
...
- **DoD**:
  - `crates/secretbank-core/src/graph.rs`
...
- **Files Touched**: `crates/secretbank-core/src/graph.rs`, `Cargo.toml` (petgraph)
...
- **DoD**:
  - `crates/secretbank-core/src/blast_radius.rs`
...
- **Files Touched**: `crates/secretbank-core/src/blast_radius.rs`
...
- **DoD**:
  - `crates/secretbank-app/src/commands/graph.rs`
...
- **Files Touched**: `crates/secretbank-app/src/commands/graph.rs`
...
- **Goal**: 증분 쿼리로 CVE 수집.
- **DoD**:
  - `crates/secretbank-feeds/src/nvd.rs`
...
- **Files Touched**: `crates/secretbank-feeds/src/nvd.rs`, `Cargo.toml`
...
- **Goal**: GHSA 목록 수집.
- **DoD**:
  - `crates/secretbank-feeds/src/ghsa.rs`
...
- **Files Touched**: `crates/secretbank-feeds/src/ghsa.rs`
...
- **DoD**:
  - `crates/secretbank-feeds/src/rss.rs`, `crates/secretbank-feeds/src/sources.rs`
...
- **Files Touched**: `crates/secretbank-feeds/src/rss.rs`, `crates/secretbank-feeds/src/sources.rs`, `crates/secretbank-feeds/tests/fixtures/rss/*.xml` (10개)
...
- **Goal**: 이메일 유출 조회.
- **DoD**:
  - `crates/secretbank-feeds/src/hibp.rs`
...
- **Files Touched**: `crates/secretbank-feeds/src/hibp.rs`, `src-tauri/Cargo.toml` (+urlencoding), `secretbank-feeds/Cargo.toml`, `secretbank-feeds/src/lib.rs`
...
- **DoD**:
  - `crates/secretbank-feeds/src/matcher.rs` — 순수 함수 (비동기 아님)
  ...
  - 기존 `secretbank-core::models::{Incident, IncidentMatch, MatchReason, Credential, Issuer}` 재사용 (신규 타입 정의 금지, `secretbank-core` path dep 추가)
  - 매칭 규칙:
...
- **Files Touched**: `crates/secretbank-feeds/src/matcher.rs`, `crates/secretbank-feeds/src/lib.rs`, `crates/secretbank-feeds/Cargo.toml` (+secretbank-core path dep)
...
- **DoD**:
  - `crates/secretbank-app/src/services/feed_scheduler.rs` + `feed_normalize.rs` (+ `services/mod.rs`)
...
- **Files Touched**: `crates/secretbank-app/src/services/feed_scheduler.rs` (신규), `feed_normalize.rs` (신규), `services/mod.rs` (신규), `context.rs` (+feed_scheduler 필드), `lib.rs` (spawn + on_window_event), `src-tauri/Cargo.toml` (+tokio-util), `secretbank-app/Cargo.toml` (+secretbank-feeds, +tokio-util)
...
- **Goal**: 프론트에 feed 제공.
- **DoD**:
  - `crates/secretbank-app/src/commands/incidents.rs` 신규 + 4 Tauri 커맨드 + `IncidentCommandError` (`#[serde(tag="code", rename_all="snake_case")]` NotFound/SchedulerUnavailable/Scheduler/Internal)
  ...
  - `IncidentFilter` DTO 신규 (`secretbank-core/src/models/incident.rs`): `source: Option<IncidentSource>`, `severity: Option<IncidentSeverity>`, `issuer_id: Option<IssuerId>`, `include_dismissed: bool` (default false — 모든 match 가 dismissed 인 incident 제외; match 없는 incident 는 포함)
...
- **Files Touched**: `crates/secretbank-app/src/commands/incidents.rs` (신규), `commands/mod.rs`, `lib.rs`, `services/feed_scheduler.rs` (+trigger_once), `secretbank-core/src/models/incident.rs` (+IncidentFilter), `secretbank-core/src/lib.rs` (re-export), `secretbank-storage/src/sqlite/repositories/incident.rs` (+3 메서드)
...
- **Files Touched**: `src/features/settings/IntegrationsSection.tsx`, `crates/secretbank-app/src/commands/settings.rs`
...
- **DoD**:
  - `crates/secretbank-connectors/src/lib.rs`
...
- **Files Touched**: `crates/secretbank-connectors/src/lib.rs`, `crates/secretbank-connectors/src/types.rs`
...
- **DoD**:
  ...
  - `crates/secretbank-connectors/src/github/mod.rs` — `GithubConnector { installation_id, relay_base_url }` 구조
- **Files Touched**: `docs/runbooks/github-app-registration.md`, `crates/secretbank-connectors/src/github/mod.rs`
...
- **Files Touched**: `secretbank-relay/src/routes/integrations_github.ts`, `crates/secretbank-connectors/src/github/auth.rs`
...
- **Files Touched**: `crates/secretbank-connectors/src/github/secret_scanning.rs`
...
- **DoD**:
  - "Connect GitHub" 버튼 → OS 기본 브라우저 열기 (`tauri_plugin_shell::open`) → `https://github.com/apps/secretbank/installations/new` → 완료 후 `Secretbank://github/callback?installation_id=...` deep link 수신
  - 연결된 installation 목록 + 저장소 리스트 + "Scan" 버튼
  - 스캔 결과를 credential inventory 와 매칭 후 새로운 것은 "Import" 제안
- **Files Touched**: `src/features/settings/GithubIntegrationSection.tsx`, `src/features/settings/use-github-integration.ts`, `crates/secretbank-app/src/commands/github.rs`
...
- **DoD**:
  - `crates/secretbank-app/src/entitlement.rs` — `fn require_pro(state: &AppState) -> Result<(), EntitlementError>`
  - Free 사용자에게는 UI 에서 쓰기 버튼을 disabled + tooltip "Pro feature"
  - 백엔드도 방어적 체크 (요청 시 Pro 아니면 거부)
  - Entitlement 정보는 릴레이 `/me` 캐시 (5분)
- **Files Touched**: `crates/secretbank-app/src/entitlement.rs`, `src/features/billing/use-entitlement.ts`
...
- **DoD**:
  - `crates/secretbank-railguard/templates/` 하위에 4개 `.tpl` 파일
  - 각 템플릿은 "이 프로젝트에서는 API 키를 하드코딩하지 말 것, 대신 .env + process.env 사용, 발견 즉시 Secretbank 로 이동" 형태의 규칙 10개
  - 프로젝트 메타(name, detected frameworks, issuer list)를 주입할 `{{variables}}`
  - `crates/secretbank-railguard/src/lib.rs` — `fn render(template: RuleKind, ctx: &Context) -> String`
- **Files Touched**: `crates/secretbank-railguard/templates/*.tpl`, `crates/secretbank-railguard/src/lib.rs`
...
- **DoD**:
  ...
  - 사용자 확인 없이 덮어쓰지 않음
- **Files Touched**: `crates/secretbank-app/src/commands/railguard.rs`
...
### T069. `secretbank-audit` 크레이트 (hash chain + ed25519)
...
- **DoD**:
  - `crates/secretbank-audit/src/chain.rs`
...
- **Files Touched**: `crates/secretbank-audit/src/chain.rs`, `Cargo.toml`
...
- **DoD**:
  - `crates/secretbank-app/src/services/device_identity.rs`
  ...
  - 있으면 로드
- **Files Touched**: `crates/secretbank-app/src/services/device_identity.rs`, `crates/secretbank-app/src/lib.rs` (setup)
...
- **DoD**:
  - `crates/secretbank-app/src/audit_ctx.rs` — `struct AuditCtx { device_key, repo }`; `fn record(action: &str, subject_kind, subject_id, payload_json) -> Result<()>`
  - 각 command 말미에 `audit_ctx.record(...)` 호출
  - transaction 기반: command 트랜잭션 안에서 audit insert 가 실패하면 전체 rollback
- **Files Touched**: `crates/secretbank-app/src/audit_ctx.rs`, 모든 mutating `commands/*.rs`
...
- **DoD**:
  - `crates/secretbank-app/src/commands/kill_switch.rs`
...
- **Files Touched**: `crates/secretbank-app/src/commands/kill_switch.rs`
...
- **Files Touched**: `src/features/issuers/IssuerDetail.tsx`, `src/features/kill-switch/BulkRevokeDialog.tsx`, `crates/secretbank-app/src/commands/kill_switch.rs` (`kill_switch_revoke_issuer`)
...
- **Priority**: Must
- **Depends on**: -
- **Goal**: `secretbank-relay/` 레포 (EE, private) 초기 구성.
- **DoD**:
...
- **Files Touched**: `secretbank-relay/` 전체 (별도 레포)
...
- **DoD**:
  - `secretbank-relay/src/db/schema.ts` — Drizzle 정의
  - `secretbank-relay/src/db/migrations/0001_init.sql`
  - `wrangler d1 migrations apply` 로 dev/prod 둘 다 실행
- **Files Touched**: `secretbank-relay/src/db/*`
...
- **Files Touched**: `secretbank-relay/src/routes/auth.ts`, `secretbank-relay/src/lib/webauthn.ts`, `secretbank-relay/src/lib/jwt.ts`
...
- **Files Touched**: `secretbank-relay/src/routes/auth.ts`, `secretbank-relay/src/lib/oauth.ts`
...
- **DoD**:
  - `crates/secretbank-app/src/commands/auth.rs`
  - Passkey: Tauri WebView 에서 `navigator.credentials.create/get` 을 사용 → 프론트가 payload 생성 후 백엔드에 전달 → 백엔드가 릴레이 호출
  - OAuth: `tauri_plugin_shell::open(auth_url)` → OS 브라우저 → deep link `Secretbank://auth/callback` → 수신 → 릴레이에 code 교환
  - JWT 는 age 볼트 파일의 `auth/session_token` 레코드 경로에 저장
- **Files Touched**: `crates/secretbank-app/src/commands/auth.rs`, `src-tauri/tauri.conf.json` (deep link scheme)
...
- **Files Touched**: `secretbank-relay/src/routes/auth.ts`, `crates/secretbank-app/src/services/session.rs`, `crates/secretbank-crypto/src/kdf.rs`
...
- **Files Touched**: `secretbank-relay/src/routes/auth.ts`, `crates/secretbank-app/src/services/session.rs`
...
- **Files Touched**: `crates/secretbank-app/src/commands/sync.rs`, `src/features/sync/SyncProvider.tsx`
...
- **Files Touched**: `src/features/sync/mapping.ts`, `crates/secretbank-app/src/commands/sync.rs` (`sync_apply_update`, emit `db:changed`)
...
- **DoD**:
  ...
  - JWT 필수
- **Files Touched**: `secretbank-relay/src/routes/sync.ts`
...
- **Files Touched**: `secretbank-relay/src/routes/sync.ts`, `secretbank-relay/src/db/schema.ts` (encrypted_secret_values), `crates/secretbank-app/src/services/value_sync.rs`, `src/features/sync/value-sync.ts`
...
- **DoD**:
  - 기존 디바이스: `sync_pair_device_start()` → X25519 ephemeral keypair 생성 + 6자리 PIN + QR (`Secretbank://pair?pk=...&pin=...&user=...`) 표시
...
- **Files Touched**: `secretbank-relay/src/routes/pair.ts`, `crates/secretbank-app/src/commands/pair.rs`, `src/features/sync/DevicePairingDialog.tsx`
...
- **Files Touched**: `secretbank-relay/src/routes/pair.ts`, `src/features/sync/DevicePairingDialog.tsx`
...
- **DoD**:
  ...
  - webhook URL: `https://relay.secretbank.app/billing/paddle/webhook`
...
- **DoD**:
  ...
  - App Store Connect 구독 그룹 `secretbank_pro_monthly` 생성 (필요: Apple Developer Program)
...
- **Files Touched**: `secretbank-relay/src/routes/billing.ts`, `secretbank-relay/src/lib/paddle.ts`
...
- **Files Touched**: `secretbank-relay/src/routes/billing.ts`, `secretbank-relay/src/lib/revenuecat.ts`
...
- **DoD**:
  ...
  - 5분 캐시, 수동 refresh 가능
- **Files Touched**: `crates/secretbank-app/src/commands/billing.rs`, `src/features/billing/use-entitlement.ts`, `src/features/billing/use-paddle-checkout.ts`
...
- **Files Touched**: `crates/secretbank-app-mobile-plugin-revenuecat/` (new mobile plugin crate), `src/features/billing/use-revenuecat.ts`
...
- **DoD**:
  ...
  - Bundle ID: `com.phoodul.Secretbank`
...
- **DoD**:
  - `crates/secretbank-storage/tests/mobile_age_vault_poc.rs` (feature-gated)
...
- **Files Touched**: `crates/secretbank-storage/src/age_vault/mod.rs` (mobile 경로 분기), `docs/runbooks/mobile-age-vault-verification.md`
...
- **Files Touched**: `crates/secretbank-app/src/commands/biometric.rs`, `src/features/vault/BiometricUnlock.tsx`
- **Tests**: manual on device
...
- **Files Touched**: `secretbank-relay/src/routes/devices.ts`, `secretbank-relay/src/lib/push.ts`, `src/features/settings/NotificationsSection.tsx`
- **Tests**: manual on device
...
- **DoD**:
  ...
  - `wrangler pages` 배포 준비 — Cloudflare Pages 프로젝트 `secretbank-web`
...
- **Priority**: Must
- **Depends on**: T110
- **Goal**: `app.secretbank.app` 에 웹 뷰어 배포.
- **DoD**:
  - `wrangler.pages.toml`
  - GitHub Actions `.github/workflows/deploy-web.yml` — main 브랜치 push 시 `pnpm build:web && wrangler pages deploy dist-web`
  - CORS: 릴레이에서 `app.secretbank.app` 허용
- **Files Touched**: `.github/workflows/deploy-web.yml`, `wrangler.pages.toml`, `secretbank-relay/src/index.ts` (CORS)
...
- **DoD**:
  - `crates/secretbank-core/src/models/rotation.rs`
...
- **Files Touched**: `crates/secretbank-core/src/models/rotation.rs`, `crates/secretbank-storage/migrations/0004_rotation_job.sql`, `crates/secretbank-storage/src/sqlite/repositories/rotation.rs`
...
- **DoD**:
  - `crates/secretbank-connectors/src/aws/mod.rs` — `AwsIamConnector` 구현 (`Connector` trait + `RotationCap::Full`)
...
- **Files Touched**: `crates/secretbank-connectors/src/aws/mod.rs`, `crates/secretbank-connectors/src/aws/iam.rs`
...
- **DoD**:
  - Stripe restricted key rolling (rolling key API): `crates/secretbank-connectors/src/stripe/`
  - GCP Service Account Key: `crates/secretbank-connectors/src/gcp/`
  - Azure Key Vault rotation: `crates/secretbank-connectors/src/azure/`
- **Files Touched**: `crates/secretbank-connectors/src/{stripe,gcp,azure}/`
...
- **Files Touched**: `src/features/rotation/ManualRotationGuide.tsx`, `crates/secretbank-app/src/commands/rotation.rs`
...
- **Files Touched**: `crates/secretbank-app/src/services/rotation_scheduler.rs`, `src/features/settings/RotationPolicySection.tsx`
...
- **Files Touched**: `crates/secretbank-app/src/commands/rotation.rs`, `src/features/rotation/RotationsPage.tsx`
...
- **Files Touched**: `src-tauri/crates/secretbank-storage/migrations/0006_credential_kind.sql`, `crates/secretbank-core/src/credential.rs`
...
- **Files Touched**: `crates/secretbank-storage/repositories/credential.rs`, schema
...
- **Files Touched**: `crates/secretbank-core/src/import.rs`, `src/features/inventory/ImportDialog.tsx`
- **Tests**: Rust — 4 형식 sample 파일 round-trip

### T-24-E. 브라우저 확장 (M24-E 풀구현으로 격상 [2026-05-09])

- **Milestone**: M24-E (M24 와 별개 마일스톤으로 격상)
- **Priority**: Tier 1 (출시 blocker)
- **Status**: ✅ **53/53 완료 (100%) — M24-E 클로즈** — Phase A 7/7 ✅ + Phase B 10/10 ✅ + Phase C 8/8 ✅ + Phase D 6/6 ✅ + Phase E 5/5 ✅ + Phase G 10/10 ✅ + Phase F 8/8 ✅ (F-3 + F-5 + F-4 + F-1 + F-2 Firefox AMO + F-7 Edge Add-ons + store matrix + **F-6 Safari placeholder [2026-05-10]** + **F-8 audit placeholder [2026-05-10]**) + GATE 1/2/2-bis 승인. **추가 보강: Icons 4 사이즈 PNG 자동 생성 [2026-05-10]** (F-1 Chrome 제출 blocker 해소). F-6 = 사용자 Apple Dev $99/년 결정 후 진행. F-8 = 1000 paid 후 Cure53 또는 NLNet NGI Zero PET 경로.
...
- **갱신 이력**: 이전 "스켈레톤 only" 정의 ([2026-05-08] Tier 1 격상 이전) 는 [2026-05-09] M24-E 직행 결정으로 풀구현 격상. project-decisions [2026-05-09] D1~D18 + Q1~Q6 일괄 승인 결과 반영.
...
2. **도메인 확보 (`secretbank.app`)**
   - 대안: `secretbank.dev`, `keyvault.dev`, `Secretbank.io`
   - 등록 비용 ~$10~30/년
...
7. **GitHub Organization 이름 (OSS + EE repo 분리)**
   - 제안: `Secretbank` (github.com/Secretbank/core, /Secretbank/relay, /Secretbank/marketing)
...
secretbank 의 두 갈래 통합:

- **Product** (사용자 시크릿 → CI 환경변수): T126~T131
- **Internal** (secretbank 자체 deploy/test): T132~T133
...
- **DoD**:
  - `crates/secretbank-connectors/src/github/actions_secrets.rs`
...
- **Files Touched**: `crates/secretbank-connectors/src/github/actions_secrets.rs`, `Cargo.toml`
...
- **Files Touched**: `crates/secretbank-app/src/commands/ci_sync.rs`, `crates/secretbank-app/src/commands/mod.rs`
...
- **DoD**:
  - `crates/secretbank-connectors/src/vercel/mod.rs` — `VercelClient` struct skeleton
  - `list_env_vars`, `upsert_env_var`, `delete_env_var` trait 정의 (미구현, `todo!()`)
  - 문서: `docs/integrations/vercel.md` — API 토큰 발급 방법, 권한 범위
- **Files Touched**: `crates/secretbank-connectors/src/vercel/mod.rs`, `docs/integrations/vercel.md`
...
- **DoD**:
  - `crates/secretbank-connectors/src/gitlab/mod.rs` — skeleton
  - `crates/secretbank-connectors/src/circleci/mod.rs` — skeleton
  - 각각 `list_variables / upsert_variable / delete_variable` trait stub
- **Files Touched**: `crates/secretbank-connectors/src/gitlab/mod.rs`, `crates/secretbank-connectors/src/circleci/mod.rs`
...
- **Files Touched**: `crates/secretbank-app/src/commands/railguard.rs`, `src/features/railguard/PreCommitSection.tsx`
...
- **Priority**: Must
- **Depends on**: -
- **Goal**: `ee/secretbank-relay/` 변경이 main 에 push 될 때 Cloudflare Workers 자동 배포.
- **DoD**:
  - `.github/workflows/deploy-relay.yml` 신규 생성
  - `paths` 필터: `ee/secretbank-relay/**` + 워크플로우 파일 자체
...
- **Priority**: Must
- **Depends on**: -
- **Goal**: 기존 CI 파이프라인에 `ee/secretbank-relay/` typecheck + test step 추가.
- **DoD**:
  - `.github/workflows/ci.yml` 에 `ee-relay` job 추가
  - `pnpm typecheck` + `pnpm test` (vitest run)
  - `pnpm-lock.yaml` 경로: `ee/secretbank-relay/pnpm-lock.yaml`
...
| Task ID | 제목 | 완료일 | 커밋 해시 |
| :------ | :--- | :----- | :-------- |
| T-23-A  | secretbank-charter crate (EFF Diceware wordlist + 6-word + 4-digit verifier codec + Shamir 2-of-3 split + XChaCha20-Poly1305 envelope + 31 회귀) | 2026-04-30 | `91ace0b` |
...
| Phase 3-A-2 | BIN 감지 + 카드 유틸 frontend (detectBrand prefix 6자만 B.5-5 / getBrandGradient oklch F.2-1 / formatCardNumber 4-4-4-4 + Amex 4-6-5 / maskCardNumber) — R1 위험 검증 Wikipedia IIN Ranges 사양 일치 (Discover 64-65 가 644-649+65 커버) / fuzz-safe NaN/비숫자/짧은입력 panic ❌ / 신규의존성 0개 순수TS / Vitest 25 PASS | 2026-05-07 | `f81e3a2` |
| Phase 3-A-3 | CreditCardVisual 3D flip 컴포넌트 (M24 3-A-3) — 앞면 (Cpu chip + brand + formatCardNumber/maskCardNumber + cardholder + 만료일 MM/YY + getBrandGradient linear-gradient) + 뒷면 (magnetic stripe + CVC 영역) 3D flip 애니메이션 (motion/react useReducedMotion hook + spring stiffness 300 damping 30, prefers-reduced-motion 시 duration=0). **GATE 2-4 구조적 보장** — flipIntent raw state + const flipped = cvcRevealed && flipIntent 파생 상태 패턴 (useEffect cleanup 불필요, react-hooks/set-state-in-effect warning 회피). F.2-1/2/3/7 모두 적용 (oklch 토큰 / prefers-reduced-motion / 키보드 role=button + tabIndex=0 + Enter/Space + aria-label). B.5-1/B.5-2 — revealedCvc / revealedCardNumber 는 prop 전달, parent 가 30s 타이머 관리 (이 컴포넌트는 표시만). backface-visibility hidden + transformStyle preserve-3d. 신규 의존성 0개 (motion v12.38.0 기존 설치). Vitest + RTL 13 PASS (V1~V12, V6 a/b 분리). | 2026-05-07 | `0194078` |
| Phase 3-A-3 | CreditCardVisual 3D flip 컴포넌트 (M24 3-A-3) — 앞면 (Cpu chip + brand + formatCardNumber/maskCardNumber + cardholder + 만료일 MM/YY + getBrandGradient linear-gradient) + 뒷면 (magnetic stripe + CVC 영역) 3D flip 애니메이션 (motion/react useReducedMotion hook + spring stiffness 300 damping 30, prefers-reduced-motion 시 duration=0). **GATE 2-4 구조적 보장** — flipIntent raw state + const flipped = cvcRevealed && flipIntent 파생 상태 패턴 (useEffect cleanup 불필요, react-hooks/set-state-in-effect warning 회피). F.2-1/2/3/7 모두 적용 (oklch 토큰 / prefers-reduced-motion / 키보드 role=button + tabIndex=0 + Enter/Space + aria-label). B.5-1/B.5-2 — revealedCvc / revealedCardNumber 는 prop 전달, parent 가 30s 타이머 관리 (이 컴포넌트는 표시만). backface-visibility hidden + transformStyle preserve-3d. 신규 의존성 0개 (motion v12.38.0 기존 설치). Vitest + RTL 13 PASS (V1~V12, V6 a/b 분리). | 2026-05-07 | `0194078` |
| Phase 3-A-4 | CreditCardForm.tsx 신규 (~313줄) — 자연 순서 폼 (카드번호 → MM/YY → CVC → 이름 → 청구주소 → 저장). react-number-format v5.4.5 PatternFormat (#### #### #### #### 또는 Amex #### ###### #####) + CVC type=password 마스킹 + Zod 스키마 (regex 숫자만 / 카드번호 13~19자 / CVC 3~4자리 / 만료 refine 과거 ❌). BIN 실시간 감지 detectBrand(cardNumber) 매 입력 + 미리보기 그레이디언트 + Amex 시 CVC 4자리 패턴. CreditCardVisual 미리보기 — cvcRevealed=false 고정 (GATE 2-4 준수). last_4/brand/cardholder/expiry 모두 입력 중 실시간 갱신. **GATE 2-5 청구주소** Textarea 선택 필드 (max 500자). **GATE 2-6 PIN ❌** (미룸). F.2-4 구체적 에러 / F.2-6 인라인 FormMessage / F.2-7 자연 탭 순서. shadcn textarea primitive 신규 생성 (기존 부재). CreateCredentialDialog 의 kind 분기 추가 (credit_card → CreditCardForm). 신규 의존성 react-number-format@5.4.5. Vitest 10 PASS (F1~F10) + CreateCredentialDialog 회귀 18 PASS. typecheck 0 / lint 신규 0 / format PASS. | 2026-05-07 | `f83295e` |
| Phase 3-A-4 | CreditCardForm.tsx 신규 (~313줄) — 자연 순서 폼 (카드번호 → MM/YY → CVC → 이름 → 청구주소 → 저장). react-number-format v5.4.5 PatternFormat (#### #### #### #### 또는 Amex #### ###### #####) + CVC type=password 마스킹 + Zod 스키마 (regex 숫자만 / 카드번호 13~19자 / CVC 3~4자리 / 만료 refine 과거 ❌). BIN 실시간 감지 detectBrand(cardNumber) 매 입력 + 미리보기 그레이디언트 + Amex 시 CVC 4자리 패턴. CreditCardVisual 미리보기 — cvcRevealed=false 고정 (GATE 2-4 준수). last_4/brand/cardholder/expiry 모두 입력 중 실시간 갱신. **GATE 2-5 청구주소** Textarea 선택 필드 (max 500자). **GATE 2-6 PIN ❌** (미룸). F.2-4 구체적 에러 / F.2-6 인라인 FormMessage / F.2-7 자연 탭 순서. shadcn textarea primitive 신규 생성 (기존 부재). CreateCredentialDialog 의 kind 분기 추가 (credit_card → CreditCardForm). 신규 의존성 react-number-format@5.4.5. Vitest 10 PASS (F1~F10) + CreateCredentialDialog 회귀 18 PASS. typecheck 0 / lint 신규 0 / format PASS. | 2026-05-07 | `f83295e` |
---
| Phase 3-A-5 | CreditCard 4 Tauri commands + Detail + 30s 자동 클리어 (M24 3-A-5) — commands/credit_card.rs 신규 (~1031 라인) — 4 Tauri commands: create_credit_card(input) (vault unlock + credential 행 + vault.put_secret(slot="card_number"/"cvc") + credit_card_meta insert + audit CreditCardCreated + CreditCardSummary 반환 B.1-3), list_credit_cards() (credit_card_meta 평문 메타만, vault unlock 불필요), reveal_card_number/cvc(credential_id) (vault.get_secret + audit + expose_secret() 1회 B.1-2 + plaintext 1회 IPC 통과 후 frontend 30s 클리어). CreditCardCommandError 2 variant (VaultLocked/Internal) 범용화 B.1-9. AuditAction 3 신규 variant (CreditCardCreated/CardNumberRevealed/CvcRevealed) — credential_id 만. CreditCardDetail.tsx 신규 (~306 라인) — CreditCardVisual + FieldRow 6필드 + 30초 자동 클리어 (REVEAL_TIMEOUT_MS=30_000, GATE 2-2/B.5-2), reveal/copy/toggle 인터랙션, unmount clearTimeout. CreateCredentialDialog의 CreditCardForm.onSubmit이 invoke("create_credit_card") 호출 통합. 신규 5 Rust 단위 테스트 (CC1~CC5+) + 신규 7 Vitest (D1~D7) PASS — 235 passed (회귀 0). clippy 0 / fmt PASS / typecheck 0. | 2026-05-07 | `a6c891a` |

| Phase 3-A-6 | BentoCard credit_card 분기 + i18n 4 로케일 (M24 3-A-6) — CreditCardBentoCard.tsx 신규 (~115줄) — Bento 컨테이너 (Card + CardContent) type-agnostic 설계 유지 + 헤더 (Lucide CreditCard 아이콘 + credential.name + SecurityBadge Phase 2-2B-5 패턴) + 카드사 그레이디언트 영역 (`getBrandGradient(brand)` linear-gradient 135deg) + `maskCardNumber(last_4, brand)` (Amex 4-6-5 / 기타 4-4-4-4, B.5-3 last_4 만) + 만료 MM/YY + brand badge i18n + hover scale-[1.01] transition 150ms (F.2-3) + role=button + Enter/Space + 디자인 토큰만 (F.2-1). `types.ts` `CredentialKind` 에 "credit_card" 추가 + `CredentialSummary` 에 card_brand / card_last_4 / card_expiry_month / card_expiry_year / card_cardholder_name 5 optional 필드 (backward compat). `BentoCard.tsx` credit_card 분기 hooks 이후 배치 (Rules of Hooks). i18n 4 로케일 `creditCard.json` (en/ko/zh/ja, 영문 fallback ❌) — title / addCard / fields / brand 7 / actions 4 / errors 3 / empty 3 = 19 키. `i18n.ts` namespace 등록. Vitest 8 신규 (B1 분기 / B2~B3 회귀 / B4 SecurityBadge 통합 / B5 visa 마스킹 / B6 amex 마스킹 / B7 클릭 / B8 brand badge) PASS — 34/34 (회귀 0). typecheck 0 / lint 신규 0 / format PASS. **이 commit 으로 Phase 3-A (신용카드 credential kind) 풀체인 완성** — 6 sub-task / 12 commits 누적. | 2026-05-07 | `f7d00a3` |
...
| T-24-E-D6 | nm-host↔Tauri TCP IPC + popup SaveDialog (Phase D 클로즈) — Part A: nm_bridge.rs (127.0.0.1:0 dynamic port + 4-byte LE length JSON 프레임 + vault unlock 시작/lock 자동 종료 Drop + SECRETBANK_BRIDGE_PORT ENV + verify_session_token HMAC-SHA256 B-7 패턴 + AuditActor::Extension audit + 3 메시지 라우팅 + vault_locked 응답). bridge_client.rs (async TCP). nm-host main.rs D-5 stub → bridge_client 위임. AppContext 17 fixture 갱신. Part B: SaveDialog.tsx 상태 머신 (loading→empty/editing→saving→saved/error) + getPendingSave 5분 TTL + issuerName 자동 매칭 + username read-only + kind 분기 credentialCreate/Update + session_expired. PendingSave Zod schema + 3 헬퍼. i18n 6키 4언어 (총 54). 신규 13 테스트 (nm_bridge 4 + SaveDialog 9), extension 320 PASS / vitest 628 PASS | 2026-05-10 | `fcbba5c` |
| T-24-E-E1 | 인라인 generator (Diceware + Random) — GeneratorIcon (autocomplete=new-password input 우측 끝 24×24 floating, getBoundingClientRect + scrollY/X 보정, z-index 2147483646) + GeneratorPanel (Diceware en/ko/ja/zh × 단어 수 4/6/8 / Random 12-64 슬라이더 + 4 문자 클래스). Closed Shadow DOM mount (D-3 패턴 재사용, z-index 2147483647). "Use this" → HTMLInputElement.prototype native value setter + InputEvent+change 이중 dispatch (React controlled input 호환). cleanup zeroString overwrite (T-CRED-1). i18n 11키 4언어 (54→65). 신규 26 테스트, 총 346 PASS | 2026-05-10 | `87809ed` |
| T-24-E-E2 | issuer recipe inheritance (preset → 휴리스틱 → 사용자 보정) — 마이그레이션 0014 issuer_recipes 테이블 (domain+source 복합 PK, recipe_json) + IssuerRecipeRepo (get_best_for_domain priority ORDER) + resolve_issuer_recipe / upsert_issuer_recipe Tauri 커맨드 + EXT_RECIPE_UPSERT audit (TM-EXT-RECIPE-AUDIT). Frontend recipe-inherit.ts (extractRecipeFromInput `[chars]{min,max}` 만 파싱 — TM-EXT-INPUT ReDoS 방어, mergeRecipes preset→userOverride→heuristic→DEFAULT_RECIPE, resolveRecipeForDomain). nm-client 2 RPC + NM 메시지 4개. GeneratorPanel mount-time 자동 적용 + userModifiedRef 추적 + "Use this" 조건부 silent upsert. 신규 32 테스트 (extension 18 + Rust 14), 회귀 0 | 2026-05-10 | `7892db0` |
| T-24-E-E3 | Site Logo (bundled SVG + S2 proxy + letter fallback) — site-logo.ts fallback chain (1 bundled SVG / 2 Google S2 favicon, 3s timeout + credentials:omit + referrerPolicy:no-referrer + URL path host 만 (TM-EXT-FAVICON-PRIVACY) / 3 letter + djb2 hash hue oklch bg) + idb-cache.ts (secretbank-cache-v1 DB, 24h TTL, chrome.storage.local fallback). 7 SVG (github/google/aws/vercel/cloudflare/openai/stripe, SimpleIcons CC0 1.0 + NOTICE attribution). SaveBanner / SaveDialog 통합 (site name 옆 logo). 신규 30 테스트 (fake-indexeddb), 총 394 PASS | 2026-05-10 | `8dc6374` |
| T-24-E-E4 | popup CredentialCard + CredentialList — Site Logo (E-3) + issuer/domain/username 카드, hover actions (Autofill/Copy username/Copy password), Enter 키 = autofill. CredentialList: chrome.tabs.query → 도메인 매칭 우선 정렬 + fuzzy 검색 + locked/empty/error/ready 상태 머신. nm-client credentialListVisible RPC + nm_bridge handle_get_credential_list (Password kind 필터, password 미포함 — T-CRED-1 metadata 만). autofill 흐름: chrome.tabs.sendMessage(autofill_credential, id) → C5 트리거 → popup window.close. NMMessage 3 타입 추가. 신규 25 테스트, 총 425 PASS | 2026-05-10 | `c5e2e39` |
| T-24-E-E5 | desktop 디자인 토큰 동기화 (Phase E 클로즈) — extension/styles/globals.css 전체 재작성 (desktop src/styles/globals.css 토큰 mirror: color 38 + radius + font, :root/.dark 구조 + @theme inline + @custom-variant dark + prefers-reduced-motion 0.01ms, light hue 250/245 / dark 252/248). Shadow DOM inline style 동기화 (single source of truth = globals.css): SaveBanner BANNER_CSS / GeneratorPanel PANEL_CSS hue 264→250/245 + destructive dark 분기 + ui-monospace + sync 시점 주석. 회귀 0 (extension 425 / vitest 628 PASS) | 2026-05-10 | `43cb49f` |
| T-24-E-G1-1 | Inline mini-graph 데이터 어댑터 (Phase G 시작 — 1P 우위 차별) — graph.rs CredentialMiniGraph + graph_for_credential Tauri command (DB 그래프 → extract_credential_mini_graph 순수 함수 1-hop subgraph + UsedBy 에지 필터 + ID 정렬 결정성 + MAX_VISIBLE_PROJECTS=5 + hidden_count + EXT_GRAPH_FETCH audit). nm_bridge dispatch + handle (TCP bridge 동일 로직). shared types/graph.ts + NMMessage 2 + nm-client graphForCredential RPC. Rust 6 + TS 3 신규 테스트, 회귀 0 (extension 428 / vitest 628) | 2026-05-10 | `8c658cd` |
| T-24-E-G1-2 | popup MiniGraph SVG fan-out + CredentialCard hover — MiniGraph.tsx 220×110 (desktop M24 1.5 패턴 이관, EDGE_RADIUS=44 / CENTER_R=10 / PROJECT_R=8 / MAX_VISIBLE=5, 12시 시작 -π/2 각도 등분, "+N more" 라벨, data-testid + role=button + Enter/Space 키보드 접근성). CredentialCard onFetchMiniGraph prop + 200ms hover delay + abortRef + hidden→loading→ready/error 상태 머신. deep-link.ts (chrome.tabs.create secretbank://graph?credential=<id> + clipboard 2단계 fallback + encodeURIComponent). 신규 18 테스트, 총 444 PASS | 2026-05-10 | `82dd99b` |
| T-24-E-G1-3 | Tauri secretbank://graph deep-link 핸들러 (G-1 풀체인 완성) — useDeepLink 훅 (@tauri-apps/api/event listen "deep-link") + handleDeepLink 순수 함수 (whitelist: scheme=secretbank: + path=graph + credential id `/^[0-9A-Za-z]{1,128}$/` — TM-DEEPLINK-INJECT 방어). DeepLinkListener BrowserRouter 내부 마운트. GraphPage useSearchParams focus → DependencyGraph focusNodeId useEffect (requestAnimationFrame setNodes focused + setCenter zoom 1.2 / 600ms). CredentialNode focused ring-2 ring-offset-2 ring-vault-warning + shadow glow. adapter.ts GraphNodeData.focused + areNodePropsEqual memo 정확도. 신규 12 테스트 (deep-link 10 + GraphPage 2), 회귀 0 (vitest 640 PASS) | 2026-05-10 | `5f4fd28` |
| T-24-E-G2-1 | Host incident 조회 백엔드 어댑터 — `incident_matches_for_host` Tauri command (IssuerRepo.domains[] + IncidentRepo 전체 조회 + match_incidents_by_host 호출, severity ≥ MEDIUM 필터 + EXT_INCIDENT_LOOKUP audit). nm_bridge `incident_check_for_host` 핸들러. nm-host main.rs bridge forward. shared types/incident.ts IncidentMatchSummary + pairing.ts NMMessageIncidentCheckForHost/Response + NMMessage union 확장. nm-client `incidentCheckForHost` RPC. matcher.rs host-only 매칭 헬퍼 (normalize_host + match_incidents_by_host + HostIncidentMatch + H-1~H-11 테스트). Rust 11+4 + TS 3 신규 테스트, 회귀 0 (extension 447 / vitest 640 PASS) | 2026-05-10 | `54d8647` |
| T-24-E-G2-2 | Supply chain in-page banner (Shadow DOM, G-2 클로즈) — SupplyChainBanner.tsx Closed Shadow DOM (T3 host JS 침범 차단) + HIGH red oklch 25° / MEDIUM amber 65° + role=alert + 자세히 보기 primary CTA → secretbank://incidents?host=<host>. banner-cache.ts 7일 dismiss + 1h cache (chrome.storage.local). supply-chain-host.ts D-3 mount 패턴 재사용. content.ts checkSupplyChainForCurrentHost + installSupplyChainHostWatcher (DOMContentLoaded + pushState/replaceState/popstate, _lastCheckedHost 중복 방지). src/lib/deep-link.ts whitelist 확장 ALLOWED_PATHS=[graph,incidents] + HOST_RE 주입 방지 (TM-DEEPLINK-INJECT). IncidentsPage useSearchParams ?host= 필터. i18n SUPPLY_CHAIN_* 6키 4언어 (65→71). 신규 33 테스트, 회귀 0 (extension 480 / vitest 640 PASS) | 2026-05-10 | `6bead7d` |
| T-24-E-G3-1 | Blast radius preview 어댑터 (host→credential) — graph.rs BlastRadiusItem (kind/label/status) + BlastRadiusForHostResponse (credential_id Option + affected + total + hidden_count) + blast_radius_for_host Tauri command (host → IssuerRepo.find_by_domain → CredentialRepo.list_all + filter Password+issuer_id → created_at DESC → 최근 credential → blast_radius BFS → MAX_VISIBLE_BLAST=5 + hidden_count, 매칭 ❌ → empty). EXT_BLAST_RADIUS_PREVIEW audit. nm_bridge dispatch + handle (TCP). nm-host bridge forward. shared types/blast-radius.ts + NMMessage 2 + nm-client blastRadiusForHost RPC. Rust 7 + TS 3 신규 테스트, 회귀 0 (extension 483 / vitest 640) | 2026-05-10 | `c4c6dd9` |
| T-24-E-G3-2 | Blast radius preview SaveBanner 통합 (G-3 클로즈) — BlastRadiusPreviewCard.tsx amber inline 카드 (kind 별 SVG 아이콘 project=Folder/deployment=Server, hidden_count 라벨, onViewDetails CTA → secretbank://graph?blast_credential=<id>). SaveBanner 통합 (blastRadius=undefined skeleton / total>0 카드 / kind=new hidden). save-handler.ts decideSaveKind 직후 kind=update 면 G-3-1 RPC 호출 + unmountRef forward reference + remountWithBlastRadius 응답 시 banner 재마운트. src/lib/deep-link.ts whitelist 확장 blast_credential 파라미터 + /graph?blast_focus=<id> + credential 동시 존재 시 blast_credential 우선. i18n BLAST_RADIUS_* 6키 4언어 (71→77). 신규 33 테스트, 회귀 0 (vitest 647) | 2026-05-10 | `7237949` |
| T-24-E-G4-1 | MCP context push (opt-in + queue + tool, 백엔드) — EXT_MCP_CONTEXT_PUSH audit. extension_settings.rs (신규) ext_settings_get/set_mcp_opt_in (SQLite settings extension_mcp_context_opt_in 기본 OFF, ES1~3 테스트). nm_bridge handle_mcp_context_push (opt-in OFF → silently drop, SQLite mcp_site_context_queue JSON array push, audit). secretbank-mcp queue.rs (신규) VecDeque capacity 10 FIFO. main.rs current_site_context tool 등록 (6번째, opt-in OFF→empty, ON→최신 N개 limit default 5/max 10). nm-host bridge forward. 프로세스 간 queue 공유 = SQLite 중간 저장소 (MVP). nm-client mcpContextPush RPC + 3 테스트. ui/switch.tsx (신규 Radix shadcn). ExtensionSettings.tsx MCP opt-in Switch + privacy 경고 + 3 테스트. i18n extensionMcpContext* 5키 4언어. 검증: cargo 290+6 PASS / vitest 650 PASS / 회귀 0 | 2026-05-10 | `c31e556` |
| T-24-E-G4-2 | MCP context push 트리거 + popup 인디케이터 (G-4 클로즈) — mcp-push.ts pushSiteContextIfEnabled (옵션 C: desktop single source of truth, 매 push 전 extSettingsGetMcpOptIn RPC + 5분 session cache + getMcpLastPush/setMcpLastPush host 별 5분 cooldown + opt-in OFF/cooldown 중 silently skip). content.ts triggerMcpPushOnPageLoad (DOMContentLoaded) + installMcpPushFormFocusListener (form input focus 첫 발생) + SPA watcher 통합 (host 변경 시 _mcpPushTriggered 리셋). popup App.tsx McpActiveIndicator (마운트 시 cache→RPC, opt-in ON → 녹색 점+"MCP" 배지, 클릭 → setActiveTab("settings") controlled Tabs 전환). storage.ts session cache 4 헬퍼. nm-client extSettingsGetMcpOptIn RPC. nm_bridge handle + nm-host forward. McpCredentialMeta 이름 충돌 해결. i18n MCP_CONTEXT_INDICATOR 4언어 (77→78). 신규 10 테스트, 회귀 0 (vitest 520 PASS) | 2026-05-10 | `a8439c0` |
| T-24-E-G5 | RAILGUARD AI 에디터 sidebar 경고 (Phase G 클로즈, M24-E 53/53 완성) — ai-editor-hosts.ts (8 host: chatgpt/cursor/cursor.sh/copilot/gemini/claude.ai/poe/perplexity, isAiEditorHost exact+subdomain). RailguardHintBanner.tsx amber sidebar 고정 (position:fixed right:12px top:12px, Closed Shadow DOM T3, z-index 2147483647, RAILGUARD 룰 생성 → secretbank://railguard + 1주 dismiss). railguard-host.ts mount 패턴. banner-cache 별도 키 secretbank_railguard_dismissed_v1 7일. content.ts checkRailguardForCurrentHost (비-AI 즉시 종료 false positive 0) + installRailguardHostWatcher (SPA pushState/replaceState/popstate). src/lib/deep-link.ts ALLOWED_PATHS = [graph,incidents,railguard]. i18n RAILGUARD_HINT_BANNER_* 4키 4언어 (78→82). 신규 62 테스트, 회귀 0 (vitest 654 / extension 582) | 2026-05-10 | `b23ac1b` |
| T-24-E-F5 | Mock Native Messaging Host (Node.js stub, F-3/F-4 backend) — tests/mock-nm-host/index.js (4-byte LE prefix + UTF-8 JSON, 1MB 상한, graceful EOF, 14 메시지 타입 stub: ping/init/pairing/credential CRUD/graph/incident/blast/MCP/recipe). install.sh (Linux Chrome/Chromium/Edge/Firefox + macOS) + install.ps1 (Windows HKCU + .cmd wrapper). fixture override SB_MOCK_FIXTURE_PATH. Node.js built-in 의존성만 (npm 0). README + 4 install/uninstall script. 신규 34 테스트 (@vitest-environment node), 회귀 0 | 2026-05-10 | `83dbf16` |
| T-24-E-F3 | Playwright E2E (Chromium MV3 옵션 B) — playwright.config.ts (headful + --load-extension + workers 1 + 60s timeout) + globalSetup/Teardown (임시 user-data-dir). fixtures.ts (Node http 가짜 login server 49152 + extensionId service_workers() URL 패턴 추출 — MV3 chrome.runtime.id content script 한계 workaround + updateNmManifestExtId Linux/macOS). 3 spec: autofill (4 smoke) + save/generator stub. .github/workflows/extension-e2e.yml (main/PR + chromium build + Xvfb + playwright install + mock-nm-host install.sh + report artifact). vitest.config.ts tests/e2e exclude. 옵션 B 단순화 (Shadow DOM selector 점진 보강) | 2026-05-10 | `48e9fa0` |
| T-24-E-F4 | Firefox build smoke + manifest validation (옵션 D) — 옵션 A web-ext+Selenium / B web-ext run / C Playwright Firefox MV3 모두 setup vs 진척 trade-off 큼, Chromium F-3 풀 E2E 가 시나리오 커버 → 옵션 D 채택 (build smoke + manifest validation 만, 풀 통합은 Phase F-2 Safari/Edge 와 함께). build-smoke.test.ts (14, child_process pnpm build:firefox + dist 검증) + manifest-validation.test.ts (20, MV2 schema + permissions + content_scripts + browser_action + gecko.id 권고) + @vitest-environment node. extension-e2e.yml firefox-build job 추가 (SKIP_FIREFOX_BUILD=true vitest + dist artifact). README 옵션 비교 + Phase F-2 풀 통합 가이드. 신규 34 테스트, 회귀 0 (총 650 PASS) | 2026-05-10 | `6344917` |
| T-24-E-F1 | Chrome Web Store submit 패키지 + nativeMessaging 권한 보강 — docs/PRIVACY.md §10 Browser Extension 신규 (10.1 NM 채널 X25519+ChaCha20-Poly1305 / 10.2 storage.local 7항목 표 / 10.3 storage.session 2 / 10.4 외부 통신 Google S2 favicons / 10.5 권한 5개 정당화). 기존 §10→§11. m24e_chrome_submission.md 10 항목 checklist (Developer 계정 / ZIP / Listing / 권한 정당화 / 스크린샷 7 / 프로모션 3 / Privacy URL / Single Purpose / 권한 audit / 거부 사유 대응). wxt.config.ts permissions nativeMessaging 추가 (B-3 NMClient 부터 사용 중 누락 발견). 사용자 액션: Developer $5 / 아이콘 PNG / 스크린샷 / 프로모션 / Listing 입력 + 제출 | 2026-05-10 | `4c50cb3` |
| T-24-E-F2+F7 | Firefox AMO + Edge Add-ons submit 패키지 + 4 store matrix — m24e_firefox_submission.md (10 항목, gecko.id secretbank@secretbank.app 필수 + Firefox manifest override + AGPL-3.0 소스코드 자동 충족 + Notes to Reviewer 통합 4 권한 정당화 + Privacy & Security 카테고리 + MV2 deprecation 대응). m24e_edge_submission.md (8 항목, chromium-mv3 100% 재사용 + Microsoft Partner Center + Edge nativeMessaging Windows 레지스트리 보강 HKCU\Microsoft\Edge\NativeMessagingHosts + Productivity 카테고리 + 정책 8 항목 비교). m24e_store_matrix.md (Chrome/Edge/Firefox/Safari 15 항목 비교 + 재사용 트리 + 비용 합산 $5/$104). DRY F-1 패턴 참조. 코드 변경 ❌ docs only | 2026-05-10 | `820de77` |
| T-24-E-F6 | Safari Xcode wrapper placeholder — docs/release/m24e_safari_submission.md 신규. Status: 보류 (사용자 결정 대기). 진입 조건 4가지 명세 (F-1/F-2/F-7 통과 + Apple Dev $99/년 결제 + macOS/Xcode 16+ + macos-latest CI 비용 수용). 자동 진행 가능 부분 plan (WXT safari target + xcrun + CI yml + 체크리스트). 대안 — Safari skip (macOS Chrome 권유). 코드 변경 ❌ docs only | 2026-05-10 | pending |
| T-24-E-F8 | 외부 audit placeholder (M24-E 클로즈) — docs/audit/m24e_phase_f_audit_plan.md 신규. B-9 결정 [2026-05-10] 옵션 C 따름: Phase F 종합 audit scope (B+C+D+E+G 전체). audit 임계점 표 + 1000 paid 전 4가지 신뢰 구축 (OSS 공개 / NLNet NGI Zero PET / 자기 검증 KAT+fuzz+CodeQL / HackerOne). 권장 로드맵 (지금~v1.0→NLNet 신청, 1000+→Cure53, 5000+→ToB). F-8 결과 분해 절차 (HIGH=블로커, MEDIUM=이관). 코드 변경 ❌ docs only | 2026-05-10 | pending |
| T-24-E-Icons | 스토어 제출 보강 — extension 아이콘 4 사이즈 PNG 생성 (16/32/48/128) — F-1 Chrome submit checklist 의 자동 가능 항목. extension/scripts/generate-icons.ps1 (System.Drawing.Bitmap HighQualityBicubic, src-tauri/icons/icon.png 512×512 원본 → extension/public/icon/{16,32,48,128}.png 생성). WXT 가 public/icon/<size>.png 자동 인식 → manifest.icons 자동 등록 검증 (chrome-mv3 + firefox-mv2 두 manifest 모두 4 사이즈 등록 확인). 회귀 0 (extension 650 PASS / manifest-validation 20/20 PASS). Chrome Web Store / Edge Add-ons / Firefox AMO 모두 동일 PNG 세트 사용 (DRY) | 2026-05-10 | `8e838e6` |
| Dependabot-postcss | Dependabot 3 moderate 해소 — postcss-rem-to-pixel (2017 마지막 release, postcss 5.x peer) → postcss-rem-to-responsive-pixel@^7.0.4 (postcss 8 peer, 2026-04 active maintained, TypeScript 재작성) 교체. extension/postcss.config.cjs 옵션 동일. 13 패키지 제거 | 2026-05-10 | `986e438` |
| CI-red-5-areas | CI red 5 영역 일괄 fix — Rust E0425 (extension_settings.rs DeviceIdentity import) + ESLint 2 errors (VaultMechanism setState→cleanup / deep-link \\- escape) + Shared TS2344 (toMatchTypeOf 방향 반전, G 단계 22 멤버 union) + Vitest unhandled rejection (graphForCredential/incidentCheckForHost 5s timeout catch silencing) + Playwright launchPersistentContext (--user-data-dir reject 대응). 9 files, 회귀 0 | 2026-05-10 | `3964cad` |
| WXT-postinstall | Extension CI typecheck fail 해소 — `defineContentScript` / `defineUnlistedScript` 등 WXT globals 가 .wxt/types/ 없으면 미정의. extension/package.json scripts 에 `postinstall: wxt prepare` 추가 (WXT 권장). pnpm install 시 5 d.ts 자동 생성 | 2026-05-10 | `10e467a` |
| Rust-migration-test | CI Rust migration test fix — idx_audit_seq cascade drop (0013 audit_log DROP+RENAME) → 0015_audit_seq_reindex.sql 신규 마이그레이션 (CREATE INDEX IF NOT EXISTS). + 0014 issuer_recipes 추가가 expected list 미반영 → migration_test.rs 의 expected tables 17→18, expected_idx 19→20 갱신. 부수: prettier --write 70 files (CI format:check 일괄 fix) | 2026-05-10 | `98d4c7e` + `021a9a3` |
