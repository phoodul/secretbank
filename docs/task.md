...
| **M24-E** | **Browser Extension (Phase A~F 풀구현 + Phase G 차별화)** | T-24-E-A1~F8 + G1-1~G5 | 53 sub-tasks | 🔄 **25/53 완료** — A 7/7 ✅ + B 10/10 ✅ + **C 8/8 ✅** (C-1 form-detector + C-2 SPA watcher + C-3 Shadow DOM + C-4 autofill + domain-match + C-5 trigger + C-6 multi-step + C-7 iframe + C-8 closed shadow + clickjack 방어) + GATE 1/2/2-bis + CI all green. **Phase G 신설 (2026-05-09)**: G-1~G-5 (모두 1P 미보유, M3/M5/M18/M20 재사용). 상세 = `docs/task_m24e.md` + `docs/task_m24e_phase_g.md`. 진입 순서: A → B → C → D → E → **G** → F-1 → F-2. **11주** 일정. |
...
- **Status**: 🔄 49/53 완료 (92%) — Phase A 7/7 ✅ + Phase B 10/10 ✅ + Phase C 8/8 ✅ + Phase D 6/6 ✅ + Phase E 5/5 ✅ + Phase G 10/10 ✅ + Phase F 3/8 (F-5 ✅ + F-3 ✅ + F-4 ✅ Firefox build smoke 옵션 D) + GATE 1/2/2-bis 승인. 다음 = F-1 (Chrome Web Store 제출 패키지 — 사용자 액션 필수).
