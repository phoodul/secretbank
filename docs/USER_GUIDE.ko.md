...
9. [CLI — `Secretbank` 명령](#9-cli--Secretbank-명령)
...
| 플랫폼  | 설치 방법                                                        |
| :------ | :--------------------------------------------------------------- |
| Windows | `secretbank_x64-setup.exe` 또는 `winget install secretbank`      |
| macOS   | `secretbank_universal.dmg` 또는 `brew install --cask secretbank` |
| Linux   | `.deb` / `.AppImage` / `.rpm` 또는 `snap install secretbank`     |

설치파일은 GitHub Releases (https://github.com/phoodul/secretbank/releases) 에서.
...
cargo install secretbank-cli      # 모든 플랫폼
```
...
# 예: Secretbank run prod-stripe -- npm run deploy
```

### 9.3 환경변수 주입 (`run`)

`Secretbank run` 은 자식 프로세스의 환경변수에 자격증명 값만 넣는다. `Secretbank.json` 설정으로 변수명 매핑:
...
- shell history 노출 우려: `Secretbank reveal` 결과를 절대 echo 하지 말고 직접 사용.
...
```sh
Secretbank mcp serve              # stdio 모드 (Claude Desktop / Cursor)
Secretbank mcp serve --port 3737  # SSE 모드 (Copilot Chat 등)
```
...
재시작 후 채팅에서 `@secretbank list openai` 같은 식으로 호출.
...
- VS Code Marketplace 에서 "Secretbank" 검색.
- 또는 Open VSX 에서 동일.

### 11.2 명령 (Command Palette)

- `Secretbank: List credentials` — Quick Pick 으로 보기.
- `Secretbank: Reveal credential` — 패스프레이즈 prompt 후 클립보드 복사.
- `Secretbank: Scan workspace for supply-chain risk` — 현재 워크스페이스 스캔.
...
- **Problems panel** — 소스 `secretbank` 로 진단 표시.
...
- 설정 → **Export encrypted backup** → `.Secretbank-backup` 파일.
...
2. **다른 vault 파일.** 기본 위치:
   - Linux: `~/.local/share/secretbank/vault.age`
   - macOS: `~/Library/Application Support/secretbank/vault.age`
   - Windows: `%APPDATA%\secretbank\vault.age`
...
| Linux (AppImage) | 압축 해제 후 `usr/bin/Secretbank`                        |
...
**Q. 기여하고 싶어요.**
A. https://github.com/phoodul/secretbank — issue / PR 환영. CLA 서명 후 머지.

**Q. 보안 취약점을 발견했어요.**
A. security@secretbank.app 으로 PGP 암호화 메일. 90일 책임 공개.
