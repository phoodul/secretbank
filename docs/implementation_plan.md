...
describe('passkey register', () => {
  it('issues challenge and stores it in KV', async () => {
    const resp = await SELF.fetch('https://relay.local/auth/passkey/register/start', {
      method: 'POST', body: JSON.stringify({ email: 't@e.st' }),
    });
...
export function SyncProvider({ children }: Props) {
  ...
  useEffect(() => {
    ...
    setDoc(d);
    return () => { secsync?.close(); persistence.destroy(); d.destroy(); };
...
export function registerCredentialsMapping(doc: Y.Doc) {
  ...
  credentialsMap.observeDeep(events => {
    suppressEcho = true;
    try {
      for (const ev of events) { /* invoke('credential_update', ...) per change */ }
...
it('Y.Map remote change does not re-emit to local', async () => { /* echo suppression */ });
```
...
it('upload and download roundtrip with encryption', async () => {
  ...
  const cipher = randomBytes(128); const nonce = randomBytes(24);
  await SELF.fetch('/sync/snapshot', { method: 'POST',
    headers: { authorization: `Bearer ${jwt}` },
    body: JSON.stringify({ doc_id: 'root', version: 1, ciphertext: b64(cipher), nonce: b64(nonce) })});
  const resp = await SELF.fetch('/sync/deltas?since=0', { headers: { authorization: `Bearer ${jwt}` }});
  const body = await resp.json();
...
app.get('/', async c => {
  ...
  const row = await c.env.DB.prepare("SELECT plan, plan_source, plan_expires_at FROM users WHERE id=?")
    .bind(user.id).first();
  const now = Date.now();
  const isActive = row.plan === 'pro' && (row.plan_expires_at == null || row.plan_expires_at > now);
  // grace period: 24h past expiry
  const inGrace = row.plan === 'pro' && row.plan_expires_at && (row.plan_expires_at > now - 86400_000);
  return c.json({
...
test('passkey login works with virtual authenticator', async ({ browser }) => {
  ...
  await client.send('WebAuthn.addVirtualAuthenticator', { options: { protocol: 'ctap2', transport: 'internal', hasResidentKey: true, hasUserVerification: true, isUserVerified: true } });
