...
Phase 3-A 는 Secretbank 에 신용카드(Credit Card) 를 새로운 `CredentialKind` 로 추가하는 작업이다. 현재 `CredentialKind` 는 `ApiKey` / `Password` 두 종류이며, Phase 3-A 를 통해 `CreditCard` 가 추가된다.
...
- `src-tauri/crates/secretbank-core/src/models/credential.rs` — `CredentialKind::CreditCard` 추가
- `src-tauri/crates/secretbank-core/src/models/credit_card.rs` (신규) — `CreditCardMeta`, `CreditCardSummary`, `CreditCardSecret`, `CreditCardInput` 구조체
- `src-tauri/crates/secretbank-storage/migrations/0012_credit_card.sql` (신규)
- `src-tauri/crates/secretbank-storage/src/sqlite/repositories/credit_card.rs` (신규)
- `src-tauri/crates/secretbank-storage/src/sqlite/repositories/mod.rs` — 모듈 등록
...
```rust
// src-tauri/crates/secretbank-core/src/models/credit_card.rs
...
const creditCardSchema = z.object({
  ...
  pin_plain: z.string().min(4).max(6).regex(/^\d+$/).optional(),
...
```json
{
  "permissions": ["core:default", "credit-card:reveal-card-number", "credit-card:reveal-cvc"]
}
```
...
_본 보고서는 LLM(claude-sonnet-4-6)이 작성한 통합 분석이다. 보안 critical 결정(B.4 항목)은 출시 전 외부 보안 감사로 검증 필수. "LLM 만 믿고 출시 ❌"._
