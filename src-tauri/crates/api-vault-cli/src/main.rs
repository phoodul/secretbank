...
//!   - `apivault add`               — credential 빠른 등록 (M24-2-4-d)
//!   - `apivault scan supply-chain` — 공급망 취약점 스캔
//!   - `apivault graph`             — 의존성 그래프 JSON 출력
//!   - `apivault blast-radius <id>` — 영향 범위 JSON 출력
//!
//! Vault 접근:
//!   - 데이터 경로 = `directories::ProjectDirs("app", "api-vault", "api-vault")`
//!     의 data_dir. Tauri 앱과 동일 경로 사용 (단일 vault).
//!   - 매 호출마다 passphrase 확인: `APIVAULT_PASSPHRASE` 환경변수 우선,
...
enum Command {
    ...
    ///
    /// Examples:
    ///   apivault add --url https://github.com --user me --name "GitHub PAT" --kind api_key
    ///   apivault add --url https://example.com  # password prompted via stdin
    Add {
...
// ---------------------------------------------------------------------------
// `apivault add` — quick credential registration
// ---------------------------------------------------------------------------
...
fn host_matches_issuer(host: &str, issuer: &api_vault_core::Issuer) -> bool {
...
async fn cmd_add(
    ...
    use api_vault_core::{CredentialId, CredentialInput, CredentialKind, Env};
    ...
    let matched_issuer: Option<&api_vault_core::Issuer> = host
        .as_deref()
...
mod tests {
    ...
    fn parse_add(args: &[&str]) -> Command {
        let cli = Cli::try_parse_from(std::iter::once("apivault").chain(args.iter().copied()))
    ...
    fn args_kind_validation_rejects_invalid() {
        let result = Cli::try_parse_from(["apivault", "add", "--pw", "x", "--kind", "invalid"]);
        assert!(result.is_err(), "clap should reject unknown --kind value");
    }

    #[test]
    fn args_env_validation_rejects_invalid() {
        let result = Cli::try_parse_from(["apivault", "add", "--pw", "x", "--env", "weird"]);
    ...
    fn make_issuer(slug: &str, domains: &[&str]) -> api_vault_core::Issuer {
        use api_vault_core::IssuerId;
        use time::OffsetDateTime;
        api_vault_core::Issuer {
            id: IssuerId::new(),
            slug: slug.to_owned(),
    ...
    fn get_passphrase_uses_env_var_and_removes_it() {
        ...
        std::env::set_var("APIVAULT_PASSPHRASE", "test-passphrase-value");
        let result = get_passphrase().expect("get_passphrase should succeed");
        use secrecy::ExposeSecret as _;
        assert_eq!(result.expose_secret(), "test-passphrase-value");
        // 환경변수 제거 확인
        assert!(
            std::env::var("APIVAULT_PASSPHRASE").is_err(),
            "APIVAULT_PASSPHRASE should be removed after get_passphrase()"
        );
    }
}
