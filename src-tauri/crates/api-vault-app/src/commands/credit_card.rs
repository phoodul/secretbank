...
use api_vault_core::models::credit_card::{
...
use api_vault_storage::sqlite::repositories::{credential::CredentialRepo, CreditCardMetaRepo};
...
impl From<api_vault_storage::sqlite::StorageError> for CreditCardCommandError {
    fn from(_: api_vault_storage::sqlite::StorageError) -> Self {
        Self::Internal
    }
}

impl From<api_vault_storage::vault::VaultError> for CreditCardCommandError {
...
pub async fn reveal_card_number(
    ...
    let secret_bytes = vault
        ...
        .await
        .map_err(|e| match e {
            api_vault_storage::vault::VaultError::NotFound { .. } => {
                CreditCardCommandError::NotFound
            }
            api_vault_storage::vault::VaultError::NotUnlocked => {
...
pub async fn reveal_cvc(
    ...
    let secret_bytes = vault.get_secret(&cvc_path).await.map_err(|e| match e {
        api_vault_storage::vault::VaultError::NotFound { .. } => CreditCardCommandError::NotFound,
        api_vault_storage::vault::VaultError::NotUnlocked => CreditCardCommandError::VaultLocked,
        _ => CreditCardCommandError::Internal,
    })?;
...
mod tests {
    ...
    use api_vault_audit::AuditActor;
    use api_vault_core::{CredentialInput, Env, IssuerId, IssuerInput};
    ...
    async fn cc2_reveal_card_number_records_audit() {
        ...
        let card_bytes =
            api_vault_storage::vault::SecretBytes::new("4111111111114242".as_bytes().to_vec());
        vault
    ...
    async fn cc5_reveal_cvc_locked_and_unlocked() {
        ...
        let cvc_bytes = api_vault_storage::vault::SecretBytes::new("321".as_bytes().to_vec());
        unlocked_vault
