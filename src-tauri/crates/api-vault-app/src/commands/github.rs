...
pub async fn github_save_installation(
    ...
    state
        .audit
        .record(
            api_vault_audit::AuditActor::LocalUser,
...
pub async fn github_scan_repo(
    ...
    let connector = GithubConnector::new(input.installation_id, "https://relay.api-vault.app");
    connector
...
#[cfg(test)]
mod tests {
    use api_vault_storage::vault::{mock::MockVaultStorage, VaultStorage as _};
    ...
    async fn raw_write(
        vault: &mut (dyn api_vault_storage::vault::VaultStorage + Send + Sync),
        list: &[GithubInstallationStored],
    ) {
