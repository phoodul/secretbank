...
use api_vault_core::{DeviceId, DeviceInput, DevicePlatform};
use api_vault_storage::sqlite::{repositories::device::DeviceRepo, SqlitePool};
...
impl From<api_vault_storage::sqlite::StorageError> for DeviceIdentityError {
    ...
    match (key_bytes_opt, id_bytes_opt) {
        ...
        (Some(key_bytes), Some(id_bytes)) => {
            ...
            match repo.get_by_id(device_id).await? {
                Some(d) if d.status == api_vault_core::DeviceStatus::Active => {
