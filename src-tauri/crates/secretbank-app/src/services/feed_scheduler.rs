...
pub enum FeedSchedulerError {
    ...
    Storage(#[from] secretbank_storage::sqlite::StorageError),

    #[error("Nvd feed error: {0}")]
    Nvd(#[from] secretbank_feeds::NvdError),

    #[error("Ghsa feed error: {0}")]
    Ghsa(#[from] secretbank_feeds::GhsaError),

    #[error("Hibp feed error: {0}")]
    Hibp(#[from] secretbank_feeds::HibpError),
}
...
mod tests {
    ...
    async fn test_poll_hibp_breaches_once_persists_incidents() {
        use secretbank_feeds::HibpClient;
        use secretbank_storage::sqlite::init_pool;
        use tempfile::tempdir;
        ...
        // DB 에 실제로 삽입됐는지 확인
        use secretbank_core::models::incident::{IncidentFilter, IncidentSource};
        use secretbank_storage::sqlite::repositories::incident::IncidentRepo;
