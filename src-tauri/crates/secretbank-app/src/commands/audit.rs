...
impl From<secretbank_storage::sqlite::StorageError> for AuditCommandError {
...
fn map_entry(log: secretbank_audit::AuditLog) -> AuditEntry {
    AuditEntry {
        id: log.id,
        seq: log.seq,
...
pub async fn audit_verify_chain(
    ...
    let mut groups: HashMap<String, Vec<secretbank_audit::AuditLog>> = HashMap::new();
    for entry in all {
...
mod tests {
    ...
    ) -> Arc<RwLock<Box<dyn secretbank_storage::vault::VaultStorage + Send + Sync>>> {
    ...
    async fn make_identity(
        vault: Arc<RwLock<Box<dyn secretbank_storage::vault::VaultStorage + Send + Sync>>>,
    ...
    async fn audit_list_empty_returns_empty() {
        ...
        let filter = secretbank_storage::AuditFilter::default();
    ...
    async fn audit_list_respects_filter() {
        ...
        let repo = AuditRepo::new(&pool);
        let filter = secretbank_storage::AuditFilter {
    ...
    async fn audit_list_limit_default() {
        ...
        let default_filter = secretbank_storage::AuditFilter {
            limit: 0,
            ..Default::default()
        };
        ...
        let big_filter = secretbank_storage::AuditFilter {
            limit: 500,
            ..Default::default()
        };
    ...
    async fn audit_verify_chain_passes_on_untampered_chain() {
        ...
        let result = secretbank_audit::verify(&all, &verifying_key);
