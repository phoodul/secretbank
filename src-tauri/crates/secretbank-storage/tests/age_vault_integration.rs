...
use secretbank_charter::{unwrap_enc_key, WrappedKey};
...
async fn recover_with_wrong_charter_fails() {
    ...
    let bogus_secret = secretbank_charter::CharterSecret::random();
    let result = vault
...
async fn recover_fails_when_vault_has_no_charter_envelope() {
    ...
    let some_secret = secretbank_charter::CharterSecret::random();
    let result = vault
