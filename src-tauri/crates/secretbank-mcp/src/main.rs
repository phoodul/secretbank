//! `Secretbank-mcp` — Model Context Protocol server for Secretbank.
//!
...
use secretbank_core::{CredentialFilter, CredentialId, CredentialStatus};
use secretbank_railguard::{render, RenderContext, RuleKind};
...
use secretbank_supply::DependencyDeclaration;

const PROTOCOL_VERSION: &str = "2024-11-05";
const SERVER_NAME: &str = "secretbank-mcp";
const SERVER_VERSION: &str = env!("CARGO_PKG_VERSION");

#[derive(Debug, Parser)]
#[command(
    name = "Secretbank-mcp",
    about = "Secretbank MCP server (Claude/Cursor/Continue) — JSON-RPC over stdio",
    long_about = "MCP server bridging Secretbank to AI agents. Register in your\n\
                  agent's MCP config (e.g. Claude Desktop's claude_desktop_config.json)\n\
                  with command 'Secretbank-mcp'."
)]
struct Cli {
...
async fn open_vault(data_dir: &std::path::Path) -> Result<Box<dyn VaultStorage + Send + Sync>> {
    ...
    eprintln!("Secretbank-mcp: vault locked. enter passphrase on the controlling terminal.");
    ...
    Ok(json!({
        "tools": [
            ...
            },
            {
                ...
                "description":
                    "Check whether the given project directory has Secretbank's RAILGUARD rule files in place. Returns the presence (true/false) of .cursorrules, .windsurfrules, CLAUDE.md, and .github/copilot-instructions.md. Use this BEFORE writing code that touches secrets — if RAILGUARD is missing, suggest the user run `Secretbank railguard apply` so AI editors stop emitting risky patterns.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "project_path": {
            ...
            },
            {
                ...
                "description":
                    "Render an Secretbank RAILGUARD template (one of cursor_rules / windsurf_rules / claude_md / copilot_instructions) for a project. Returns the file content as text — caller can show it to the user before writing. Useful when AI editor lacks RAILGUARD and the user wants a preview before committing.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "kind": {
...
async fn tool_check_supply_chain_risk(args: &Value) -> Result<Value, RpcError> {
    ...
    let mut advisories: Vec<secretbank_supply::PackageAdvisory> = Vec::new();
...
async fn tool_check_railguard_status(args: &Value) -> Result<Value, RpcError> {
    ...
    let summary = if all_present {
        ...
        "RAILGUARD: missing files — recommend `Secretbank railguard apply` or use suggest_railguard_template to preview.".to_string()
    };
    let json_text =
...
async fn dispatch(state: &ServerState, line: &str) -> Option<String> {
    let req: RpcRequest = match serde_json::from_str(line) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("Secretbank-mcp: bad request: {e}");
            return None;
        }
    };
    ...
    if req.id.is_none() {
        match req.method.as_str() {
            ...
            other => {
                eprintln!("Secretbank-mcp: ignoring notification {other}");
                return None;
            }
        }
    }
...
async fn main() -> Result<()> {
    ...
    eprintln!("Secretbank-mcp: ready (protocol {PROTOCOL_VERSION})");
