// Entry point — delegates to secretbank_app::run()
// Keeps the binary target where Tauri CLI expects it (src-tauri/src/main.rs)
// while the actual runtime logic lives in the secretbank-app library crate.
//
...
fn main() {
    secretbank_app::run(tauri::generate_context!());
}
